 ...
 /*-- / Coded by ARVK --*/	
	...
	function get_products($id_product)
   {
	...
	function get_SMS_OTPdata($service_id,$customer_data){
        ...
        $resultset = $this->db->query("SELECT sms_msg, sms_footer, template_name from services where id_services = '".$service_id."'");
		foreach($resultset->result() as $row)
		{
			$sms_msg = $row->sms_msg;
			$sms_footer = $row->sms_footer;
		...
		$sms_msg .= " ".$sms_footer;
		return [ "message" => $sms_msg, "params" => $params, "template_name" => $template_name ];
    }
