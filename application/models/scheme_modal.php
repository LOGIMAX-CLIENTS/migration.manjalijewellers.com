		...
		foreach ($classifications as $clsfy) {
			if ($branchwise_scheme == 1) {
				$sch_sql = $this->db->query("SELECT id_classification,sb.id_branch,b.name
		    								...
		    								WHERE " . ($id_branch != '' && $is_branchwise_cus_reg == 1 ? ' sb.id_branch=' . $id_branch . ' and'  : '') . " s.is_digi = 0 and s.active=1 and s.visible=1 and id_classification=" . $clsfy['id_classification'] . " GROUP BY id_classification,id_branch");
			} else {
...
ifnull(p.add_charges,0.00) as add_charges,
	...
	function insert_paymentData_bymobile($data)
	{
		foreach ($data as $key => $val) {
			...
			if ($resultset->num_rows() > 0) {
				...
				foreach ($resultset->result() as $row) {
					$records = array(
						'id_scheme_account' => $val['insertID'],
	...
	function get_kyc_details()
	{
		...
		if ($result->num_rows() > 0) {
			...
			$data['bank']['status'] = (($b_stat == 0) ? 'Pending' : (($b_stat == 1) ? 'In Progress' : (($b_stat == 2) ? 'Verified' : 'Rejected')));
			$data['pan']['status'] = (($p_stat == 0) ? 'Pending' : (($p_stat == 1) ? 'In Progress' : (($p_stat == 2) ? 'Verified' : 'Rejected')));
			$data['aadhar']['status'] = (($a_stat == 0) ? 'Pending' : (($a_stat == 1) ? 'In Progress' : (($a_stat == 2) ? 'Verified' : 'Rejected')));
			$data['dl']['status'] = (($d_stat == 0) ? 'Pending' : (($d_stat == 1) ? 'In Progress' : (($d_stat == 2) ? 'Verified' : 'Rejected')));
		} else {
			$data['bank'] = array(
				"bank_name" 	=> NULL,
				"number" 		=> NULL,
	...
	function get_plan_detail($id_sch_ac)
	{
		$sql = $this->db->query("SELECT 
					...
					s.scheme_name, s.code, s.scheme_type, s.min_amount, s.max_amount, s.min_weight, s.max_weight,
					IFNULL(s.auto_debit_plan_type, 0) as auto_debit_plan_type,
					IFNULL(s.amount, 0) as scheme_amount,
