	...
	function syncPayData($ac_data) 
	{
		...
		foreach($ac_data as $data){	 
			...
			if($resultset->num_rows() > 0 ){
				...
				foreach($resultset->result() as $row)
				{
				    $checkRef_no = $this->db->query("SELECT receipt_no,payment_ref_number from payment where is_offline=1 and payment_ref_number ='".$row->ref_no."' and receipt_no ='".$row->receipt_no."'");
				   // echo $this->db->last_query();exit;
				    if($checkRef_no->num_rows() == 0)
				    {
    				    if($row->is_modified == 0){
                            ...
                            $update_existing_pmd = $this->payment_modal->updData($update_pmd,'id_payment',$this->db->insert_id(),'payment_mode_details');
                            ...
                            $ins_cycle = $this->payment_modal->get_due_date('service', $dt_pay,$data['id_sch_ac']);  
                            ...
                            if(sizeof($ins_cycle[0]) > 0){
                                ...
                                $this->payment_modal->updData($cycle_data, 'id_payment', $this->db->insert_id(), 'payment');
                            ...
                            if(sizeof($paid) > 0){
                                ...
                                $this->payment_modal->updData($paid_ins, 'id_scheme_account', $data['id_sch_ac'] , 'scheme_account');
				...
				if($this->config->item("integrationType") == 3){ // Jewelone ERP integration
			        $no_records += 1; 
