...
Left Join customer c on (c.id_customer=sa.id_customer)
	...
	p.payment_type,p.payment_mode,cs.allow_referral,s.agent_refferal,s.agent_credit_type,s.emp_refferal,sa.referal_code,s.firstPayment_as_wgt,sa.firstpayment_wgt,p.metal_weight,p.redeemed_amount,wa.id_wallet_account,p.date_payment 
        ...
        $sql1 = "Select p.due_type, (select e.id_employee from employee e where e.emp_code = sa.referal_code and sa.referal_code != '' and sa.referal_code is not null) as ref_emp_id,sa.id_customer,firstPayment_amt,s.code as group_code,s.sync_scheme_code,sa.id_branch as branch,cs.scheme_wise_acc_no,cs.gent_clientid,s.firstPayamt_as_payamt,s.firstPayamt_maxpayable,p.id_adv_payment,sa.id_scheme_account,sa.scheme_acc_number,sa.id_scheme,cs.schemeacc_no_set,cs.receipt_no_set,cs.scheme_wise_receipt,p.ref_trans_id,cs.edit_custom_entry_date,sa.custom_entry_date,p.payment_amount,flexible_sch_type,p.id_branch,s.is_lucky_draw, s.max_members, s.code,s.one_time_premium,p.id_transaction,b.warehouse,
	p.payment_ref_number,p.gst_type,IFNULL(p.gst_amount,0) as gst_amount,IFNULL(p.discountAmt,0) as discountAmt,IFNULL(p.actual_trans_amt,0) as actual_trans_amt,
    ...
    /**
     * getSyncSchemeData — exact replica of admin payment_model->getSchemeData()
     * fields consumed by the post-payment saveall blocks:
     *   one_time_premium / rate_fix_by / rate_select  → rate-fix logic
     *   maturity_type / maturity_days                 → start_date + maturity calculation
     *   firstPayamt_as_payamt / firstPayamt_maxpayable → firstPayment_amt capture
     *   firstPayment_as_wgt                           → firstpayment_wgt capture
     *   paid_installments (inline subquery)           → 'first payment' guard (== 0 check)
     */
    function getSyncSchemeData($id_sch_acc)
    {
        $sql = $this->db->query("
            SELECT
                /* scheme flags — same columns as admin payment_model getSchemeData */
                s.maturity_type,
                s.maturity_days,
                s.installment_cycle,
                s.total_installments,
                s.ins_days_duration,
                s.interest,
                s.apply_benefit_by_chart,
                s.one_time_premium,
                s.rate_fix_by,
                s.rate_select,
                s.firstPayamt_as_payamt,
                s.firstPayamt_maxpayable,
                ...
                sa.fixed_wgt,
                sa.is_opening,
                sa.paid_installments   AS opening_paid_installments,
                /* calc_maturity_date — same expression as admin model */
                date_add(
                    Date_format(sa.start_date, '%Y-%m-%d'),
                    INTERVAL s.maturity_days DAY
                ) AS calc_maturity_date,
                /* paid_installments — same subquery logic as admin getPaidInsData */
                IFNULL(
                    (SELECT
                        if(s2.is_topup_scheme = 1,
                           MAX(p.installment),
                           COUNT(DISTINCT p.installment)
                        )
                     FROM payment p
                     JOIN scheme s2 ON s2.id_scheme = sa.id_scheme
                     WHERE p.id_scheme_account = sa.id_scheme_account
                       AND p.payment_status = 1
                    ), 0
                ) AS paid_installments
            FROM scheme_account sa
            LEFT JOIN scheme s ON s.id_scheme = sa.id_scheme
            WHERE sa.id_scheme_account = " . (int)$id_sch_acc);
        return $sql->row_array();
    }

    /**
     * isSyncRateFixed — mirrors mobileapi_model->isRateFixed()
     * Returns ['status' => 1] if rate already fixed, 0 if not yet.
     */
    function isSyncRateFixed($id_sch_ac)
    {
        $sql = $this->db->query(
            "SELECT fixed_wgt, firstPayment_amt FROM scheme_account WHERE id_scheme_account = " . (int)$id_sch_ac
        );
        $row = $sql->row();
        return array(
            'status'           => ($row->fixed_wgt > 0 ? 1 : 0),
