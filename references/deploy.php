...
$ENVIRONMENTS = [
    'Retail_1.1.1.0001' => [
        ...
        'name' => 'Develop',
        'maintenance' => false,
        'deploy_mode' => 'git_pull'  // or 'symlink' for zero-downtime
    ],
    'QA' => [
        'folder' => 'qa',
        'name' => 'QA',
...
$CONFIG = [
    ...
    'base_path' => getenv('BASE_PATH') ?: '/home/retaillogimaxind/public_html',
    'log_file' => getenv('LOG_FILE') ?: '/home/retaillogimaxind/public_html/webhooks/webhook.log',
...
$token_paths = [
    '/home/retaillogimaxind/public_html/webhooks/.github_token',
    '/home/retaillogimaxind/.github_token'
];
...
if ($deploy_mode === 'symlink' && file_exists($unified_script)) {
    ...
    $success = strpos($output, 'Deployment complete') !== false 
            || strpos($output, '✅') !== false 
...
// =============================================================================
// RUN DATABASE MIGRATIONS (after code pull, before notifications)
// =============================================================================
...
if ($success) {
    ...
    // Config path differs by environment:
    //   dev/qa/support → {env_path}/global_configs.php (in-folder)
    //   production     → shared/config/global_configs.php (centralized)
    if ($folder === 'etail') {
        $config_file = $CONFIG['base_path'] . '/shared/config/global_configs.php';
    } else {
        $config_file = $env_path . '/global_configs.php';
    }

    if (file_exists($migrate_script) && is_dir($migrate_dir)) {
        $migrate_env = ($folder === 'etail') ? 'production' : strtolower($env_name);
        $backup_flag = ($folder === 'etail') ? '--backup' : '--no-backup';
        $migrate_cmd = sprintf(
            'bash %s --config %s --migrations %s --env %s %s 2>&1',
            escapeshellarg($migrate_script),
            escapeshellarg($config_file),
            escapeshellarg($migrate_dir),
            escapeshellarg($migrate_env),
            $backup_flag
        );
        $migrate_output = shell_exec($migrate_cmd);
        log_event('INFO', "Migration ($migrate_env): " . substr($migrate_output, 0, 500));
    } else {
        log_event('WARNING', "Migration skipped — script or directory missing");
    }
}
...
if (!empty($CONFIG['github_token'])) {
    // Send different event types based on deploy mode
    $event_type = ($deploy_mode === 'symlink') ? 'production-deployed' : 'staging-deployed';
...
// =============================================================================
// LOG TO CI/CD DASHBOARD DATABASE
// =============================================================================

$api_url = 'https://retail.logimaxindia.com/develop/cicd/docs/api/deployment-history.php';
...
if ($api_code === 200 || $api_code === 201) {
    log_event('INFO', 'Logged to dashboard DB ✓');
} else {
    log_event('WARNING', "Dashboard DB log failed (HTTP $api_code)");
}
