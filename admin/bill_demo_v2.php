<?php
$conn = mysqli_connect('localhost', 'root', 'root@123', 'retaillogimaxind_etailv3');
if (mysqli_connect_error()) {
    die("Connection failed: " . mysqli_connect_error());
}

$block_html = '
<table style="width:100%; border-collapse:collapse; font-size: 10px; font-family: DejaVu Sans, sans-serif;" class="jewelry-detailed-table">
...
$stmt = $conn->prepare("INSERT INTO print_template_custom_blocks (block_name, block_html, block_category) VALUES (?, ?, 'Legacy Layouts') ON DUPLICATE KEY UPDATE block_html = VALUES(block_html)");
if ($stmt) {
...
$stmt2 = $conn->prepare("INSERT INTO print_templates (template_code, template_name, template_category, template_html, is_active, gjs_data) VALUES ('BILL_FMT_2_VISUAL', 'Bill Format 2 (Visual Designer)', '1', ?, 1, '{}') ON DUPLICATE KEY UPDATE template_name = VALUES(template_name)");
if ($stmt2) {
    $stmt2->bind_param("s", $template_html);
    $stmt2->execute();
    echo "SUCCESS_INSERT";
} else {
    echo "Error inserting template: " . mysqli_error($conn);
}
?>
