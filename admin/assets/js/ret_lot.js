...

{
	...
	var from_date = $("#lt_date1").html();

	var to_date = $("#lt_date2").html();

	var report_name = "Lot Inward Report"
	
	var optional = '';

	var header = generateReportHeader(report_name, from_date, to_date, optional);
	                ...
	                "buttons": [
						{
							...
							footer: true,
							title: '',
							messageTop: header.htmlTitle,
							customize: function (win) {
								$(win.document.body).find('table')
									.addClass('compact')
									.css('font-size', '10px');

								$(win.document.body).find('table tfoot td, table tfoot th').attr(
	                ...
	                "aoColumns": [
								...
								{ "mDataProp": function ( row, type, val, meta ) {
									...
									var total_pcs=0;
				                	 ...
				                	 edit_url=((access.edit=='1' && row.lot_from==1 && row.is_closed != 1) ? base_url+'index.php/admin_ret_lot/lot_inward/edit/'+id : '#' );
									...
									(row.lot_from == 1 && row.is_closed != 1 && access.edit == 1 && row.is_tag_lot == 1 ? 
				            ...
				            "footerCallback": function (row, data, start, end, display) {
        						...
        						if (data.length > 0) {
        							...
        							for (var i = 0; i <= data.length - 1; i++) {
        								...
        								$(api.column(7).footer()).html(money_format_india(parseFloat(pcs).toFixed(0)));
        								...
        								$(api.column(8).footer()).html(money_format_india(parseFloat(grs_wgt).toFixed(3)));
        								...
        								$(api.column(9).footer()).html(money_format_india(parseFloat(tag_pcs).toFixed(0)));
        								...
        								$(api.column(10).footer()).html(money_format_india(parseFloat(tag_gwt).toFixed(3)));
        								...
        								$(api.column(13).footer()).html(money_format_india(parseFloat(blc_pcs).toFixed(0)));
        								...
        								$(api.column(14).footer()).html(money_format_india(parseFloat(blc_wt).toFixed(3)));
        								...
        								$(api.column(15).footer()).html((parseFloat(col_14).toFixed(3)));
...

			{
				...
				$('#lot_det > tbody tr').each(function(bidx, brow){
...

					{
						...
						if( list_data[0].lot_no == lot_id.find('.lot_id').val()){
...

				{
					...
					$.each(list_data,function(key,item)
...

					{
						...
						trHtml +='<tr id='+i+'>'
							...
							+ ((data.access.delete == '1') ? '<td><a href="#" onClick="remove_lot_row($(this).closest(\'tr\'));" class="btn btn-danger btn-del"><i class="fa fa-trash"></i></a></td>' : '')
							+ '</tr>';
...

{
	...
	if((total_lotted_wgt == tot_lotNew_wgt))
...

{
	...
	row += '<tr id='+i+'>'
		...
		+'<td><input type="number" step="any" value="'+$('#lot_det > tbody tr:first td.l_lot_pcs').text()+'" name="merge_item['+i+'][pcs]" class="lm_pcs" style="width:60px;" readonly></td>'
...
function add_lot_details(){
	...
	var lot_details={
		...
		'tagged_count':$('#lot_tagged_count').val(),
...
function edit_lot_details(curRow){
	...
	if($('#lot_inward_details_id').val() != '' || $('#select_product').val() != '' && $('#select_product').val() != null)
	{
		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"Please complete the current item edit before selecting another one."});
		return false;
	}
	...
	$('#lot_tagged_count').val(lot_details.tagged_count);
	...
	// Reset disabled/readonly states

	$('#select_section, #select_product, #select_design, #select_sub_design, #calculation_based_on, #lot_net_wt, #mc_value, #mc_type, #lot_wastage').prop('disabled', false).prop('readonly', false);

	$('#lot_pcs, #lot_gross_wt').prop('readonly', false);

	$('.add_tag_lwt, .add_other_metals, .add_other_charges').show().css('pointer-events', 'auto').css('opacity', '1');



	if(lot_details.tagged_count > 0)

	{

		$('#select_section, #select_product, #select_design, #select_sub_design, #calculation_based_on, #lot_net_wt, #mc_value, #mc_type, #lot_wastage').prop('disabled', true);

		// $('.add_other_metals, .add_other_charges').css('pointer-events', 'none').css('opacity', '0.5');

		// Note: gross_wt, pcs, and less_wt (lot_lwt) remain editable as per requirement
