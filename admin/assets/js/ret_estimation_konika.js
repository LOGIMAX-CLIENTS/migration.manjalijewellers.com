...
+ '<td><div class="huid">' + data[0].huid + '</div></td>'
								...
								+ '<td><input type="text" class="form-control market_rate_value" name="est_tag[est_rate_per_grm][]" ' + rate_readonly + ' value="' + rate_per_grm + '" style="width:100px;" /><div class="tag_pure_rate_display"></div></td>'
						...
						if ($('#estimation_tag_details >tbody > tr').length > 0) {
							...
							if (!rowExist) {
								...
								row += '<tr>'
									...
									+ '<td><input type="text" class="market_rate_value" name="est_tag[est_rate_per_grm][]" ' + rate_readonly + ' value="' + rate_per_grm + '" /><div class="tag_pure_rate_display"></div></td>'
...
function create_new_empty_est_tag_row() {
	...
	row += '<tr id="' + rowId + '">'
		...
		+ '<td><input type="text" class="market_rate_value" name="est_tag[est_rate_per_grm][]" ' + rate_readonly + '  value="0" /><div class="tag_pure_rate_display"></div></td>'
...
function create_new_empty_est_catalog_row() {
	...
	row += '<tr id=' + i + '>'
		...
		+ '<td><input type="number" class="form-control cat_nwt" name="est_catalog[nwt][]" value="" autocomplete="off" readonly style="width:80px;"><div class="cat_nwt_display"></div></td>'

		+ '<td><input class="form-control cat_market_rate_value" name="est_catalog[est_rate_per_grm][]" type="text" value="" style="width:80px;" ' + rate_readonly + '  /><div class="cat_pure_rate_display"></div></td>'
...
function create_new_empty_est_custom_row() {
	...
	row += '<tr id=cus' + i + '>'
		...
		+ '<td><input type="number" class="form-control cus_nwt" name="est_custom[nwt][]"  readonly style="width:80px;"/><div class="cus_nwt_display"></div></td>'

		+ '<td><input class="form-control cus_market_rate_value" name="est_custom[est_rate_per_grm][]" type="text" value="" style="width:80px;" ' + rate_readonly + '  /><div class="cus_pure_rate_display"></div></td>'
...
function validateTagDetailRow() {
	...
	$('#estimation_tag_details > tbody  > tr').each(function (index, tr) {
		...
		let total_mc_va = calculate_total_mc_va(rate, cal_type, gwt, nwt, wastage_max_per, mc_cal_type, act_mc_value);

		let total_min_mc_va = calculate_total_mc_va(rate, cal_type, gwt, nwt, min_va, min_mc_cal_type, min_mc);
...
function validateOldMatelDetailRow() {
	...
	$('#estimation_old_matel_details > tbody  > tr').each(function (index, tr) {
		...
		if ($(this).find('.old_item_type').val() == "" || $(this).find('.old_id_category').val() == "" || $(this).find('.old_id_category').val() == null || $(this).find('.old_metal_category').val() == "" || $(this).find('.old_metal_category').val() == null || $(this).find('.old_gwt').val() == "" || $(this).find('.old_pcs').val() == "" || $(this).find('.old_metal_type').val() == "" || $(this).find('.old_metal_type').val() == null || $(this).find('.old_purity').val() == "" || $(this).find('.old_purity').val() == 0 || $(this).find('.old_purity').val() > 100) {
...
function validateVoucherDetailRow() {
 ...
 $(document).on('change',".is_eda",function(){
...

    {
        ...
        $('#iseda').val(0);
        $('.cus_nwt_display, .cat_nwt_display, .cus_pure_rate_display, .cat_pure_rate_display, .tag_pure_rate_display, .tag_nwt_display').html('');
...
function calculateSaleValue() {
	...
	$('#estimation_catalog_details > tbody tr').each(function (idx, row) {
		...
		*/
		...
		if (calculation_type == 0) {
			...
			((retail_max_mc / 100) * (parseFloat(rate_per_grm) * (parseFloat(wast_wgt) + parseFloat(net_wt)))) : parseFloat(retail_max_mc * 1)));
	...
	else if (is_eda_est == 1) {
		// Pure Weight Calculation for EDA
		var purity = parseFloat(curRow.find('.cat_purity option:selected').text().trim());
		var wastage_per = parseFloat(curRow.find('.cat_wastage').val());
		if (isNaN(purity)) { purity = 0; }
		if (isNaN(wastage_per)) { wastage_per = 0; }
		var pure_rate = parseFloat($(".goldrate_24ct").text().trim());
		if (isNaN(pure_rate)) { pure_rate = 0; }
		var pure_wt = parseFloat((net_wt * (purity + wastage_per)) / 100).toFixed(3);
		// MC still applies in EDA
		var mc_type_val = parseFloat(curRow.find('.id_mc_type').val() == 2 ? parseFloat(retail_max_mc * net_wt) : parseFloat(retail_max_mc * 1));
		if (isNaN(mc_type_val)) { mc_type_val = 0; }
		rate_with_mc = parseFloat(parseFloat(pure_wt * pure_rate) + parseFloat(mc_type_val)).toFixed(2);
		market_rate_with_mc = rate_with_mc;
		total_tax_rate = 0;
		market_total_tax_rate = 0;
	}
		...
		if (is_eda_est == 1) {
			// Redefining pure_wt here for display
			var purity = parseFloat(curRow.find('.cat_purity option:selected').text().trim());
			var wastage_per = parseFloat(curRow.find('.cat_wastage').val());
			if (isNaN(purity)) { purity = 0; }
			if (isNaN(wastage_per)) { wastage_per = 0; }
			...
			if (isNaN(pure_rate)) { pure_rate = 0; }
			var pure_wt = parseFloat((net_wt * (purity + wastage_per)) / 100).toFixed(3);
			curRow.find('.cat_nwt_display').html('<small>Pure Wt: ' + pure_wt + '</small>');
			curRow.find('.cat_pure_rate_display').html('<small>Pure Rate: ' + pure_rate + '</small>');
			curRow.find('.cat_pure_wt').val(pure_wt);
			curRow.find('.cat_pure_rate').val(pure_rate);
		} else {
			curRow.find('.cat_nwt_display').empty();
			curRow.find('.cat_pure_wt').val('');
			curRow.find('.cat_pure_rate').val('');
		}
...
function calculateCustomItemSaleValue(curRow) {
	...
	$('#estimation_custom_details > tbody tr').each(function (idx, row) {
		...
		// Pure Weight Calculation for EDA (Home Bill)
		var purity = parseFloat(curRow.find('.cus_purity option:selected').text().trim());
		var wastage_per = parseFloat(curRow.find('.cus_wastage').val());
		if (isNaN(purity)) { purity = 0; }
		if (isNaN(wastage_per)) { wastage_per = 0; }
		var pure_rate = parseFloat($(".goldrate_24ct").text().trim());
		if (isNaN(pure_rate)) { pure_rate = 0; }
		...
		if (is_eda_est == 1) {
			// Redefining pure_wt here for the display part
			var purity = parseFloat(curRow.find('.cus_purity option:selected').text().trim());
			var wastage_per = parseFloat(curRow.find('.cus_wastage').val());
			if (isNaN(purity)) { purity = 0; }
			if (isNaN(wastage_per)) { wastage_per = 0; }
			...
			if (isNaN(pure_rate)) { pure_rate = 0; }
			var pure_wt = parseFloat((net_wt * (purity + wastage_per)) / 100).toFixed(3);
			curRow.find('.cus_nwt_display').html('<small>Pure Wt: ' + pure_wt + '</small>');
			curRow.find('.cus_pure_rate_display').html('<small>Pure Rate: ' + pure_rate + '</small>');
			curRow.find('.cus_pure_wt').val(pure_wt);
			curRow.find('.cus_pure_rate').val(pure_rate);
		} else {
			curRow.find('.cus_nwt_display').empty();
			curRow.find('.cus_pure_wt').val('');
			curRow.find('.cus_pure_rate').val('');
		}
...
function calculatetag_SaleValue() {
	...
	$('#estimation_tag_details > tbody tr').each(function (idx, row) {
		...
		if (curRow.find('.est_tag_id').val() != '') {
			...
			if (curRow.find('.nwt').is('input')) {
				curRow.find('.nwt').val(net_wt);
			} else {
			...
			*/
			...
			if (calculation_type == 0) {
				...
				wast_amt = parseFloat(parseFloat(wast_wgt) * parseFloat(board_rate)).toFixed(2);
...

        	{
					...
					curRow.find('.pure_wt').val('');
					curRow.find('.pure_rate').val('');

			}

			else if (is_eda_est == 1) {
				// Pure Weight Calculation for EDA (Tags)
				var purity = NaN;
				// Try select dropdown first (some tag rows use <select class="tag_purity">)
				if (curRow.find('.tag_purity').length > 0) {
					purity = parseFloat(curRow.find('.tag_purity option:selected').text().trim());
				}
				// Then try div.purity text (some tag rows use <div class="purity">)
				if (isNaN(purity) && curRow.find('div.purity').length > 0) {
					purity = parseFloat(curRow.find('div.purity').text().trim());
				}
				if (isNaN(purity)) { purity = 0; }
				var wastage_per = parseFloat(curRow.find('.wastage_max_per').val());
				if (isNaN(wastage_per)) { wastage_per = 0; }
				var pure_rate = parseFloat($(".goldrate_24ct").text().trim());
				if (isNaN(pure_rate)) { pure_rate = 0; }
				var pure_wt = parseFloat((net_wt * (purity + wastage_per)) / 100).toFixed(3);
				// MC still applies
				var mc_val = isNaN(parseFloat(mc_type)) ? 0 : parseFloat(mc_type);
				rate_with_charges = parseFloat(parseFloat(pure_wt * pure_rate) + parseFloat(mc_val)).toFixed(2);
				total_price = rate_with_charges;
				market_total_price = rate_with_charges;				
				total_tax_rate = 0;
				// Display Pure WT info under N.Wt
				if (curRow.find('.nwt').is('input')) {
					curRow.find('.nwt').val(net_wt);
					curRow.find('.tag_nwt_display').html('<small>Pure Wt: ' + pure_wt + '</small>');
				} else {
					curRow.find('.nwt').html(net_wt + '<br><small>Pure Wt: ' + pure_wt + '</small>');
				}
				curRow.find('.tag_pure_rate_display').html('<small>Pure Rate: ' + pure_rate + '</small>');
				curRow.find('.pure_wt').val(pure_wt);
				curRow.find('.pure_rate').val(pure_rate);
			}
...
function calculate_total_mc_va(rate_per_grm, calculation_type, gross_wt, net_wt, tot_wastage, id_mc_type, mc) {
	...
	if (calculation_type == 0) {
		...
		if (id_mc_type != 3) {
			...
			mc = parseFloat(id_mc_type) == 2 ? parseFloat(mc * gross_wt) : parseFloat(mc * 1);
		...
		if (id_mc_type != 3) {
			...
			mc = parseFloat(id_mc_type) == 2 ? parseFloat(mc * net_wt) : parseFloat(mc * 1);
			...
			success: function (data) {
				...
				if (data.tag_details.item_details != null && data.tag_details.item_details.length > 0) {
					...
					if(!topup_scheme) {
						...
						row += '<tr id="' + rowId + '">'
							...
							+ '<td><input type="text" class="form-control market_rate_value" ' + rate_readonly + ' name="est_tag[est_rate_per_grm][]" value="' + item.est_rate_per_grm + '"style="width:100px;" /><div class="tag_pure_rate_display"></div></td>'
				...
				if (data.non_tag_details.item_details != null && data.non_tag_details.item_details.length > 0) {
					...
					$.each(data.non_tag_details.item_details, function (key, item) {
						...
						row += '<tr id=' + i + '>'
							...
							+ '<td><input type="number" class="form-control cat_nwt" name="est_catalog[nwt][]"value=' + item.net_wt + ' autocomplete="off" readonly style="width:80px;"><div class="cat_nwt_display"></div></td>'

							+ '<td><input class="form-control cat_market_rate_value" name="est_catalog[est_rate_per_grm][]" ' + rate_readonly + ' type="text" value=' + item.est_rate_per_grm + ' style="width:80px;"/><div class="cat_pure_rate_display"></div></td>'
				...
				if (data.est_home_bill.item_details != null && data.est_home_bill.item_details.length > 0) {
					...
					$.each(data.est_home_bill.item_details, function (key, item) {
						...
						select_section = item.section == null ? "<option value=''>Select Section</option>" : '';

						$.each(section_details, function (key, eitem) {

							var selected = "";

							if (eitem.is_home_bill_counter == 1) {

								if (item.id_section == eitem.id_section) {

									selected = "selected";

								}

								select_section += "<option value='" + eitem.id_section + "' " + selected + ">" + eitem.section_name + "</option>";	// alert(1)
						...
						row += '<tr id=cus' + i + '>'
							...
							+ '<td><select class="form-control cus_section" name="est_custom[id_section][]" value="" placeholder="Search Section" style="width:150px;">'+select_section+'</select><input type="hidden" class="cus_section_id"  value=' + item.id_section + ' /></td>'
							...
							+ '<td><select class="form-control item_emp_id" style="width:100px !important" name="est_custom[item_emp_id][]" value=' + item.item_emp_id + ' >' + select_emp + '</select></td>'
							...
							+ '<td><input type="number" class="form-control cus_gwt" name="est_custom[gwt][]"  placeholder="Enter GWT" value=' + item.gross_wt + ' style="width:80px;"/><input type="hidden" class="form-control blc_tag_gwt" value="'+ item.gross_wt +'"/></td>'

							+ '<td><div class="form-group"><div class="input-group "><input class="form-control cus_lwt" onclick="create_new_empty_est_cus_stone_item($(this).closest(\'tr\'));" type="number" value=' + item.less_wt + ' name="est_custom[lwt][]" step="any" readonly="" style="width:100px;"><span class="input-group-addon input-sm add_tag_lwt" onclick="create_new_empty_est_cus_stone_item($(this).closest(\'tr\'));">+</span><input type="hidden" class="stone_details" name="est_custom[stone_details][]"  value=' + JSON.stringify(stone_details) + ' ><input type="hidden" class="stone_price" name="est_custom[stone_price][]" value=' + stone_price + ' ></div></div></td>'

							+ '<td><input type="number" class="form-control tot_stn_amt" name="est_custom[stone_amt][]" value=' + item.stn_amt + ' readonly style="width:100px;" /></td>'

							+ '<td><input type="number" class="form-control tot_dia_amt" name="est_custom[diamond_amt][]" value=' + item.dia_amt + ' readonly style="width:100px;" /></td>'

							+ '<td><input type="number" class="form-control cus_nwt" name="est_custom[nwt][]" value=' + item.net_wt + ' readonly style="width:80px;"/><div class="cus_nwt_display"></div></td>'

							+ '<td><input class="form-control cus_market_rate_value" name="est_custom[est_rate_per_grm][]" type="text" ' + rate_readonly + ' value= ' + item.est_rate_per_grm + ' style="width:80px;"/><div class="cus_pure_rate_display"></div></td>'

							+ '<td><select class="form-control cus_mc_type" style="width:80px;"><option value="2" ' + (item.mc_type == 2 ? 'selected' : '') + '>Gram</option><option value="1" ' + (item.mc_type == 1 ? 'selected' : '') + '>Piece</option><option value="3" ' +(item.mc_type == 3 ? 'selected' : '') + '>% on Price</option></select><input type="hidden" value="' + item.mc_type + '" name="est_custom[id_mc_type][]" class="id_mc_type"></td>'

							+ '<td><input type="number" class="form-control cus_mc" name="est_custom[mc][]"value=' + item.mc_value + ' placeholder="MC" style="width:80px;"/><input type="hidden" value="' + item.max_mc + '" name="est_custom[max_mc][]" class="HB_cus_mc"></td>'

							+ '<td><input type="number"  class="form-control cus_act_va_per"  name="est_custom[max_va_per][]" style="width:80px;"  readonly  value=' + item.max_va_per + ' placeholder="VA in %" /><input type="hidden" class="cus_pro_va_per"></td>'

							+ '<td><input class="form-control cus_wastage" type="number" name="est_custom[wastage][]" placeholder="VA in %" value=' + item.wastage_percent + ' style="width:80px;"/><input type="hidden" class="slab_details" value=' + JSON.stringify(slab_details) + '><input type="hidden" class="act_va_per" value="' + item.act_wast_per + '" name="est_custom[act_va_per][]" ><input type="hidden" class="id_wastage_slab" value="' + item.id_wastage + '"  name="est_custom[id_wastage_slab][]"><input type="hidden" class="wast_slb_val" name="est_custom[wast_slab_val][]" value=' + item.slb_val + '></td>'
							...
							// + '<td><input type="number" class="form-control cus_taxable_amt" name="est_custom[taxable_amt][]" readonly="" value=' + parseFloat(parseFloat(item.item_cost) - parseFloat(item.item_total_tax)).toFixed(2) + ' style="width:80px;"></td>'

							+ '<td><input type="number" class="form-control cus_taxable_amt" name="est_custom[taxable_amt][]"' +(item.calculation_based_on == 3 || item.calculation_based_on == 4 ? 'readonly' : '') + 'value=' + item.item_total_tax + ' style="width:80px;"></td>'
							...
							+ '<td><input class="form-control cus_amount" type="number" name="est_custom[amount][]" value=' + item.item_cost + ' readonly style="width:100px;"/><input type="hidden" class="cus_calculation_based_on" name="est_custom[calculation_based_on][]" value=' + item.calculation_based_on + ' /><input type="hidden" id="material_details" class="material_details" name="est_custom[material_details][]"><input type="hidden" value=' + stone_price + ' class="stone_price" id="stone_price" value="stone_price"><input type="hidden" value="0" class="material_price" id="material_price"><input type="hidden" class="market_rate_cost" name="est_custom[market_rate_cost][]"value=' + item.market_rate_cost + '><input type="hidden" class="market_rate_tax" name="est_custom[market_rate_tax][]"><input type="hidden" name="est_tag[total_cgst][]" class="cus_cgst"/><input type="hidden" class="cus_sgst" name="est_tag[total_sgst][]"/><input type="hidden" class="cus_igst" name="est_tag[total_igst][]"/><input type="hidden" class="cus_taxable_amount" name="est_tag[sale_taxable_amount][]"/><input type="hidden" class="cus_pure_wt" name="est_custom[pure_wt][]" value="' + (item.pure_wt || '') + '"><input type="hidden" class="cus_pure_rate" name="est_custom[pure_rate][]" value="' + (item.pure_rate || '') + '"></td>'
						...
						$('#estimation_custom_details > tbody').find('.cus_product,.cus_design,.cus_sub_design,.cus_purity,.item_emp_id').select2();
						...
						let lastRow = $('#estimation_custom_details > tbody tr:last');
		...
		$.each(tag_details, function (pkey, pitem) {
			...
			if (!rowExist) {
				...
				row += '<tr id="' + rowId + '">'
					...
					+ '<td><input type="text" class="market_rate_value form-control" name="est_child_tag[est_rate_per_grm][]" ' + rate_readonly + ' value="' + pitem.est_rate_per_grm + '" /><div class="tag_pure_rate_display"></div></td>'
...
function set_tag_split_details() {
	...
	var scheme_avg_rate = total_closing_amount > 0
		...
		$.each(tag_split_details, function (key, val) {
			...
			if (total_closing_weight > 0 && (!isTagsplitted)) {
				...
				if ((parseFloat(total_balance_applied_weight) < parseFloat(item_weight)) && (total_balance_applied_weight > 0) && (val.calculation_based_on != 3) && (val.id_metal == 1) && (parseFloat(curRow.find('.bonus_percent').val()) > 0)) { //&& (val.id_metal == 1) (parseFloat(total_balance_applied_weight) < parseFloat(item_weight)) &&
	            	...
	            	var gross_wt = (split_wt_type == 1 ? total_balance_applied_weight : parseFloat(parseFloat(total_balance_applied_weight) + parseFloat(tag_stn_weight) + parseFloat(other_wt)).toFixed(3));

                    var net_wt = (split_wt_type == 1 ? (parseFloat(total_balance_applied_weight) - parseFloat(other_wt) - parseFloat(tag_stn_weight)) :(parseFloat(total_balance_applied_weight)));
			...
			if (val.balance_weight > 0) {
				...
				*/
				...
				tag_details.push({
					...
					'gross_wt'						:(split_wt_type == 1 ? parseFloat(item_balance_weight).toFixed(3) : parseFloat(parseFloat(item_balance_weight) + parseFloat(other_wt) + parseFloat(tag_stn_weight)).toFixed(3) ),
...
function append_tag_details(val)

	{
		...
		row += '<tr id="' + rowId + '">'
			...
			+ '<td><span class="tag_purity_name" style="width:100px;">' + (Number(val.purname).toFixed(2)) + '</span><input class="tag_id_purity" type="hidden" name="est_tag[purity][]" value="' + val.purity +'"/></td>'
			...
			+ '<td><input type="text" class="form-control gwt" name="est_tag[gwt][]" step="any" value=' + val.gross_wt + ' readonly style="width:80px;" /><input type="hidden" class="cur_gwt" name="est_tag[cur_gwt][]" value=' + val.gross_wt + ' /><input type="hidden" class="act_gwt" value=' + val.gross_wt + ' /></td>'

			//+'<td><input type="text" class="lwt" name="est_tag[lwt][]" step="any" value='+val.less_wt+' readonly/></td>'

			+ '<td><div class="form-group"><div class="input-group "><input class="form-control lwt" value=' + val.less_wt + ' onClick="create_new_empty_est_cus_stone_item($(this).closest(\'tr\'));"  type="number" name="est_tag[lwt][]" step="any" readonly style="width:80px;"/><span class="input-group-addon input-sm add_tag_lwt" onClick="create_new_empty_est_cus_stone_item($(this).closest(\'tr\'));">+</span></div></div><input type="hidden" class="stone_details" name="est_tag[stone_details][]" value=' + JSON.stringify(stone_details) + '><input type="hidden" class="stone_price" name="est_tag[stone_price][]"></td>'
			...
			+ '<td><input type="text" class="form-control nwt" value=' + val.net_wt + ' readonly style="width:100px;" /><div class="tag_nwt_display"></div><input type="hidden" class="form-control tot_nwt" name="est_tag[nwt][]" value=' + val.net_wt + ' /><input type="hidden" class="tot_tag_nwt"  value=' + val.net_wt + ' style="width:100px;" /></td>'

			+ '<td><input type="text" class="form-control market_rate_value" name="est_tag[est_rate_per_grm][]" ' + rate_readonly + ' value="' + (rate_per_grm > 0 ? rate_per_grm : val.sales_value) + '" style="width:100px;" /><div class="tag_pure_rate_display"></div></td>'
			...
			+'<td>'+(val.tag_other_itm_amount > 0 ? '<a href="#" onClick="showTagothermetals($(this).closest(\'tr\'));">'+ parseFloat(tag_other_full_amt.tot_amount).toFixed(2) +'</a>' :val.tag_other_itm_amount)+'<input type="hidden" class="tag_other_metal_details" value=' + JSON.stringify(other_metal_details) + '></td>'
...

	{
		...
		$.each(other_metal_details,function(key,val){
			...
			row+='<tr>'
				 ...
				 +'<td>'+val.tag_other_itm_wastage+'</td>'

				 +'<td>'+(val.tag_other_itm_cal_type==1 ? 'Per Gram':'Per Piece')+'</td>'

				 +'<td>'+val.tag_other_itm_mc+'</td>'

				 +'<td>'+ parseFloat(calcData.rate_per_gram).toFixed(2)+'</td>'

				 +'<td>'+ parseFloat(calcData.tot_amount).toFixed(2)+'</td>'
