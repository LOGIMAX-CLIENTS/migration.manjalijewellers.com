...
$('#get_duplicate_tag').on('click',function(){
...

	{
        ...
        $.ajax({
            ...
            data: { 'FromDt': FromDt, 'ToDt': $('#rpt_to_date').html() ,'tag_lot_id':tag_lot_id,'id_branch':$('#id_branch').val(),'tag_id':$('#tag_id').val(),'id_product':$('#prod_select').val(),'des_select':$('#des_select').val(),'from_weight':from_weight,'to_weight':to_weight,'tag_code':$('#tag_no').val(),'old_tag_code':$('#old_tag_no').val()},
...

			{
...

				{
					...
					if(items.wastag_method == 1) {
						...
						if (items.wastag_value != null && items.wastag_value !== "") {
						...
						if (items.wastag_value_wt != null && items.wastag_value_wt !== "") {
							$('#tag_wast_perc').prop('readonly', true);
							$('#tag_wast_value').prop('readonly', true);
							$('#tag_id_mc_type').prop('disabled', true);
							$('#tag_id_mc_type').css('pointer-events', 'none');
					...
					$.each(items.weight_range_det,function(i,result){
						...
						if((parseFloat(result.wc_from_weight) <= parseFloat($('#tag_gwt').val())) && (parseFloat($('#tag_gwt').val()) <= parseFloat(result.wc_to_weight))){
							...
							if(result.wc_method == 1) {
								...
								if (result.wc_percent != null && result.wc_percent !== "") {
									$('#tag_wast_perc').prop('readonly', true);
									$('#tag_wast_value').prop('readonly', true);
									$('#tag_id_mc_type').prop('disabled', true);
									$('#tag_id_mc_type').css('pointer-events', 'none');
