...
class Syncapi_model extends CI_Model
{
    ...
    function getcustomerByStatus($trans_status,$id_branch,$record_to,$tran_date="",$from_date="",$to_date=""){	
        if($id_branch == ''){
	        $sql = "SELECT * FROM customer_reg WHERE id_branch is null and record_to='$record_to' and is_transferred='$trans_status'";
	    ...
	    if(!empty($from_date) && !empty($to_date)){
			$sql .= " and (DATE(transfer_date) BETWEEN '".$this->db->escape_str($from_date)."' AND '".$this->db->escape_str($to_date)."')";
		}elseif($tran_date != ""){
			$sql .= " and transfer_date='".$this->db->escape_str($tran_date)."'";
		}
		$sql .= " limit 1000";
		return $this->db->query($sql)->result_array();
    } 
...
/*	function getTransactionByTranStatus($trans_status,$id_branch,$record_to,$tran_date="")
	...
	function getRegisteredAccTransactions($trans_status, $id_branch, $record_to, $tran_date = "", $from_date = "", $to_date = "")
	{
	...
	function getCustomerByID($id_scheme_account)
	{
		$sql = "SELECT
					   ...
					   sa.id_scheme_account as  id_scheme_account,ifnull(c.id_branch,sa.id_branch) as id_branch,maturity_date,
