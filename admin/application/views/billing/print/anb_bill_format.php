...
* BILL_FORMAT_3 is designed for scenarios where MC and VA should not be displayed.
 * Do not modify BILL_FORMAT_2 to hide MC/VA; switch to BILL_FORMAT_3 instead.
 
 * BILL FORMAT 3 (Simplified/Unified View)
 * ----------------------------------------
 * PURPOSE:
 * This format provides a cleaner, more simplified receipt for the customer.
 * It removes the explicit columns for 'Making Charges' (MC) and 'Value Addition' (VA).
 * 
 * BUSINESS CONTEXT:
 * Use this format for customers who prefer a simple receipt or when the business 
 * strategy is to present a unified rate/amount without itemizing overhead and labor costs.
 * The layout is adjusted to utilize the extra horizontal space for the description.
 */
...
<head>
	...
	<style>
		...
		* {
		...
		/* Replace flexbox with table layout for DomPDF compatibility */
		.wrapper {
			width: 100%;
			table-layout: fixed;
		...
		@page {
			size: A4;
			margin-top: 5cm;      /* Skip top pre-printed design */
			margin-bottom: 7.5cm; /* Skip bottom pre-printed design */
			margin-left: 25px;
			margin-right: 25px;
		}

		/* Table styles optimized for DomPDF */
		table {
			width: 100%;
			border-collapse: collapse;
			font-size: 10px;
		...
		.payment-mode-details td,
		.payment-mode-details th {
		...
		.col-5-percent { width: 5%; }
		.col-8-percent { width: 8%; }
		.col-10-percent { width: 10%; }
		.col-11-percent { width: 11%; }
		.col-15-percent { width: 15%; }
		.col-26-percent { width: 26%; }
		.col-30-percent { width: 30%; }

		/* Money symbol styling */
		.rupee-symbol {
			font-size: 15px !important;
		}

		/* Additional DomPDF fixes */
		img {
			max-width: 100%;
			height: auto;
		}
		...
		/* Softcopy background image */
		<?php if ($is_softcopy && isset($background_image)) { ?>
		body {
...
<?php if (sizeof($receiptDetails) > 0) {
	...
	foreach ($receiptDetails as $val) {
		$adj_amt += $val['adjuseted_amt'];
		$refund_amt += $val['refund_amt'];
	}
	$adj_amt = number_format($adj_amt + $refund_amt, 2, '.', '');
		...
		<table class=paging>
				...
				<tr>
					<td>
						<div class="content-block">
							...
							<!-- Replace flexbox wrapper with table layout for DomPDF compatibility -->
							<table class="wrapper-table">
								<tr>
									<td class="col-40" id="a1" style="font-weight: bold;">
									<?php if ($billing['billing_for'] == 1 || $billing['billing_for'] == 2) { ?>
										...
										<div style="font-weight: bold;">
											...
											<label><?php echo ($billing['address2'] != '' ?  strtoupper($billing['address2']) . ',' : ''); ?></label><br>
                                     	...
                                     	<label><?php echo ($billing['cus_country'] != '' ? 'Country:&nbsp;&nbsp;' . strtoupper($billing['cus_country']) : ''); ?></label><br> -->
											...
											<label><?php echo (isset($billing['passport_no']) && $billing['passport_no'] != '' ? '<br>PASSPORT NO:&nbsp;&nbsp;' . strtoupper($billing['passport_no']) . '' : ''); ?></label>
											<label><?php echo (isset($billing['driving_license_no']) && $billing['driving_license_no'] != '' ? '<br>DRIVING LICENSE NO:&nbsp;&nbsp;' . strtoupper($billing['driving_license_no']) . '' : ''); ?></label>
									...
									?>
										...
										<?php } else { ?>
											<?php if (!$is_softcopy && isset($qrfilename)) { ?>
												<img src="<?php echo base_url(); ?>bill_qrcode/<?php echo $qrfilename; ?>.png" alt="QRcode" style="height:65px; width:65px;">
											<?php } ?>
									<?php } } ?>
									</td>
									<td class="col-5" id="a2sub"></td>
									<td class="col-35" id="a3">
									<div>
										<table style="font-weight:bold; width:100%; border-collapse:collapse;">
											<tr>
												<td style="width:40%; padding:2px;"><label><?php echo ($billing['bill_type'] == 15 ? 'Ack No' : ($billing['is_eda']==1 ? 'Invoice No' : 'Proforma No')); ?></label></td>
												<td style="width:10%; padding:2px;">:</td>
												<td style="width:50%; padding:2px;"><?php echo $inv_no; ?></td>
											</tr>
											<tr>
												<td style="padding:2px;"><label>Date</label></td>
												<td style="padding:2px;">:</td>
												<td style="padding:2px;"><?php echo $billing['bill_date'] ?></td>
											</tr>
											<tr>
												<td style="padding:2px;"><label>Time</label></td>
												<td style="padding:2px;">:</td>
												<td style="padding:2px;"><?php echo $billing['bill_time']; ?></td>
											</tr>
											<tr>
												<td style="padding:2px;"><label>Gold 22-KT</label></td>
												<td style="padding:2px;">:</td>
												<td style="padding:2px;"><?php echo  number_format($gold_metal_rate, 2, '.', '') . '/Gm'; ?></td>
											</tr>
											<!-- <tr>
												<td style="padding:2px;"><label>Gold 18-KT</label></td>
												<td style="padding:2px;">:</td>
												<td style="padding:2px;"><?php echo  number_format($billing['goldrate_18ct'], 2, '.', '') . '/Gm'; ?></td>
											</tr> -->
											<tr>
												<td style="padding:2px;"><label>SILVER</label></td>
												<td style="padding:2px;">:</td>
												<td style="padding:2px;"><?php echo number_format($silver_metal_rate, 2, '.', '') . '/Gm'; ?></td>
											</tr>
											<?php if ($billing['is_eda'] == 1) { ?>
											<tr>
												<td style="padding:2px;"><label>GSTIN</label></td>
												<td style="padding:2px;">:</td>
												<td style="padding:2px;"><?php echo $comp_details['gst_number']; ?></td>
											</tr>
											<?php } ?>
											...
											<!-- <tr>
												...
												<td>:</td>
												<td><?php //echo ($billing['delivered_at'] == 1 ? ( $billing['bill_type']==13 ? $billing['branch_name'] : 'Showroom') : 'Customer Address'); ?></td>
											</tr> -->
							...
							<div>
								...
								<div for="title" style="height:20px;text-align:center;font-weight:bold;"><?php if($billing['bill_status']==2){echo $title." - CANCELLED";} else {echo $title;}?></div> <br>
			...
			<tbody>
				<tr>
					<td>
						...
						<div class="content-wrapper" style="margin-top:-30px;">
							<div class="box">
								<div class="box-body">
											...
											<?php if (sizeof($est_other_item['item_details']) > 0) { ?>
												...
												<div class="col-xs-12">
													<div class="table-responsive">
														<table id="pp" class="table" style="table-layout: fixed;">
															<tr>
																<td colspan="8">
														...
														<table id="pp" style="width:100%; border-collapse:collapse; table-layout: fixed; font-size:10px;">
															<thead style="text-transform:uppercase;font-size:10px;">
																<tr>
																	<th class="table_heading alignCenter" style="width: 5%; text-align:center; padding:3px; border:none;">S.No</th>
																	<th class="table_heading alignLeft" style="width: 33%; text-align:left; padding:3px; border:none;">Description</th>
																	<th class="table_heading alignRight" style="width: 10%; text-align:right; padding:3px; border:none;">HSN</th>
																	<th class="table_heading alignRight" style="width: 5%; text-align:right; padding:3px; border:none;">PCS</th>
																	<th class="table_heading alignRight" style="width: 10%; text-align:right; padding:3px; border:none;">GRS.WT</th>
																	<th class="table_heading alignRight" style="width: 10%; text-align:right; padding:3px; border:none;">NET.WT</th>
																	<th class="table_heading alignRight" style="width: 12%; text-align:right; padding:3px; border:none;">Rate</th>
																	<th class="table_heading alignRight" style="width: 15%; text-align:right; padding:3px; border:none;">Amount</th>
																</tr>
															</thead>
															...
															<tbody>
																...
																?>
																	...
																	<tr>
																		<td style="text-align:center; padding:3px; border:none; width:5%;"><?php echo $i; ?></td>
																		<td style="text-align:left; padding:3px; border:none; width:33%; white-space:nowrap; overflow:hidden; font-weight: bold;"><?php echo $items['design_name']; ?></td>
																		<td style="text-align:right; padding:3px; border:none; width:10%;"><?php echo $items['hsn_code']; ?></td>
																		<td style="text-align:right; padding:3px; border:none; width:5%;"><?php echo $items['piece']; ?></td>
																		<td style="text-align:right; padding:3px; border:none; width:10%;"><?php echo $items['gross_wt'] . ($items['pro_uom'] != '' ? '-' . $items['pro_uom'] : ''); ?></td>
																		<td style="text-align:right; padding:3px; border:none; width:10%;"><?php echo $items['net_wt'] . ($items['pro_uom'] != '' ? '-' . $items['pro_uom'] : '') ?></td>
																		<td style="text-align:right; padding:3px; border:none; width:12%;"><?php echo moneyFormatIndia($items['rate_per_grm']); ?></td>
																		<td style="text-align:right; padding:3px; border:none; width:15%;"><?php echo moneyFormatIndia(number_format((float)($item_taxable > 0 ? ($item_taxable - $other_metal_amount) : 0), 2, '.', '')); ?></td>
																	</tr>
																	<!-- <tr>
															<td></td>
															...
															<td colspan="6"></td>
																	...
																	<?php if (sizeof($items['other_metal_details']) > 0) { ?>
																		...
																		<?php foreach ($items['other_metal_details'] as $other_metal_details) { ?>
																			<tr style>
																				<td></td>
																				<td colspan="4"><?php echo $other_metal_details['metal'] . ' (Gwt: ' . $other_metal_details['tag_other_itm_grs_weight'] . ' / Wst: ' . $other_metal_details['tag_other_itm_wastage'] . '% / MC: ' . $other_metal_details['tag_other_itm_mc'] . ')' ?></td>
																	...
																	?>
																			<tr class="stones">
																				...
																				<?php if ($stoneItems['stone_cal_type'] == 1) { ?>
																					<td colspan="5" class='textOverflowHidden stoneData'><?php echo $stoneItems['pieces'] . ' Pcs ' . $stoneItems['stone_name'] . ($stoneItems['amount'] > 0 ? ' ' . number_format($stoneItems['wt'], 3, '.', '') . ' ' . $stoneItems['uom_short_code'] . 'x' . moneyFormatIndia(number_format($stoneItems['rate_per_gram'], 2, '.', '')) . '=' . moneyFormatIndia(number_format((float)($stoneItems['amount']), 2, '.', '')) : ''); ?></td>
																				<?php } else if ($stoneItems['stone_cal_type'] == 2) { ?>
																					<td colspan="5" class='textOverflowHidden stoneData'><?php echo number_format($stoneItems['wt'], 3, '.', '') . ' ' . $stoneItems['uom_short_code'] . ' ' . $stoneItems['stone_name'] . ($stoneItems['amount'] > 0 ? ' ' . $stoneItems['pieces'] . 'Pcs ' . 'x' . $stoneItems['rate_per_gram'] . '=' . moneyFormatIndia(number_format((float)($stoneItems['amount']), 2, '.', '')) : ''); ?></td>
																				<?php } ?>
															...
															<tr style="font-weight: bold">
																<td style="text-align:center; padding:3px; border:none; width:5%;">Total</td>
																<td style="padding:3px; border:none; width:33%;"></td>
																<td style="padding:3px; border:none; width:10%;"></td>
																<td style="text-align:right; padding:3px; border:none; width:5%;"><?php echo $pieces; ?></td>
																<td style="text-align:right; padding:3px; border:none; width:10%;"><?php echo number_format($gross_wt, 3, '.', ''); ?></td>
																<td style="text-align:right; padding:3px; border:none; width:10%;"><?php echo number_format($net_wt, 3, '.', ''); ?></td>
																<td style="padding:3px; border:none; width:12%;"></td>
																<td style="text-align:right; padding:3px; border:none; width:15%;"><?php echo moneyFormatIndia(number_format((float)($taxable_amt), 2, '.', '')); ?></td>
															</tr>
															<tr>
																<td colspan="10">
																	<hr class="item_dashed">
															...
															<?php if ($taxable_amt > 0) { ?>
																<tr>
																	<td colspan="6" style="border:none; font-weight: bold;" class="alignRight">SUB TOTAL</td>
																	<td class="alignRight" style="font-size:15px !important; font-weight: bold;">&#8377;</td>
																	<td class="alignRight" style="font-weight: bold;"><?php echo moneyFormatIndia(number_format((float)($taxable_amt), 2, '.', '')); ?></td>
																</tr>
															<?php } ?>
															<?php if ($total_sgst > 0) { ?>
																<tr>
																	<td colspan="6" style="border:none; font-weight: bold;" class="alignRight">SGST<?php echo (sizeof($est_other_item['tax_details']) == 1 ? '(' . ($est_other_item['item_details'][0]['tax_percentage'] / 2) . '%)' : '') ?></td>
																	<td class="alignRight" style="font-size:15px !important; font-weight: bold;">&#8377;</td>
																	<td class="alignRight" style="font-weight: bold;"><?php echo moneyFormatIndia(number_format((float)($total_sgst), 2, '.', '')); ?></td>
																</tr>
															<?php } ?>

															<?php if ($total_cgst > 0) { ?>
																<tr>
																	<td colspan="6" style="border:none; font-weight: bold;" class="alignRight">CGST <?php echo (sizeof($est_other_item['tax_details']) == 1 ? '(' . ($est_other_item['item_details'][0]['tax_percentage'] / 2) . '%)' : '') ?></td>
																	<td class="alignRight" style="font-size:15px !important; font-weight: bold;">&#8377;</td>
																	<td class="alignRight" style="font-weight: bold;"><?php echo moneyFormatIndia(number_format((float)($total_cgst), 2, '.', '')); ?></td>
																</tr>
															<?php } ?>

															<?php if ($total_igst > 0) { ?>
																<tr>
																	<td colspan="6" style="border:none;" class="alignRight">IGST <?php echo (sizeof($est_other_item['tax_details']) == 1 ? '(' . ($est_other_item['item_details'][0]['tax_percentage']) . '%)' : '') ?></td>
															...
															<?php if ($scheme_benefit > 0) { ?>
																<tr>
																	<td colspan="6"></td>
																	<td class="alignRight">CHIT BENEFIT<span style="margin-left: 10%;">&#8377;</span></td>
															...
															<?php if ($billing['handling_charges'] > 0) { ?>
																<tr>
																	<td colspan="6"></td>
																	<td class="alignRight">H.C<span style="margin-left: 10%;">&#8377;</span></td>
															...
															if ((sizeof($est_other_item['return_details']) == 0) && ((sizeof($est_other_item['old_matel_details']) == 0)) && ($billing['round_off_amt'] != 0)) { ?>
																<tr>
																	<td colspan="6" class="alignRight" style="font-weight: bold;">Round Off</td>
																	<td class="alignRight" style="font-size:15px !important; font-weight: bold;">&#8377;</td>
																	<td class="alignRight" style="font-weight: bold;"><?php echo moneyFormatIndia(number_format($billing['round_off_amt'], 2, '.', '')); ?></td>
																</tr>
																<tr>
																	<td colspan="7"></td>
																	<td>
																...
																<tr style="font-weight: bold;">
																	<td colspan="6" class="alignRight" style="font-size: 12px;">TOTAL</td>
																	<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
																	<td class="alignRight" style="font-size: 12px;"><?php echo moneyFormatIndia(number_format($tot_sales_amt + $billing['handling_charges'] + $billing['round_off_amt'] - $scheme_benefit, 2, '.', '')); ?></td>
																</tr>
																<tr>
																	<td colspan="7"></td>
																	<td >
															...
															<?php if ($billing['tcs_tax_amt'] > 0) { ?>
																<tr>
																	<td colspan="6"></td>
																	<td class="alignRight">TCS (<?php echo $billing['tcs_tax_per'] . '%'; ?>)<span style="margin-left: 10%;">&#8377;</span></td>
									...
									<?php if (sizeof($est_other_item['return_details']) > 0) { ?>
										...
										<table id="pp" style="width:100%; border-collapse:collapse; font-size:10px;table-layout: fixed;">
											<thead style="text-transform:uppercase;font-size:10px;">
												<tr>
													<th class="table_heading alignCenter" style="width: 5%; text-align:center; padding:3px; border:none;">S.No</th>
													<th class="table_heading alignLeft" style="width: 42%; text-align:left; padding:3px; border:none;">Description</th>
													<th class="table_heading alignRight" style="width: 8%; text-align:right; padding:3px; border:none;">HSN</th>
													<th class="table_heading alignRight" style="width: 5%; text-align:right; padding:3px; border:none;">PCS</th>
													<th class="table_heading alignRight" style="width: 9%; text-align:right; padding:3px; border:none;">GRS.WT</th>
													<th class="table_heading alignRight" style="width: 9%; text-align:right; padding:3px; border:none;">NET.WT</th>
													<th class="table_heading alignRight" style="width: 10%; text-align:right; padding:3px; border:none;">Rate</th>
													<th class="table_heading alignRight" style="width: 12%; text-align:right; padding:3px; border:none;">Amount</th>
												</tr>
											</thead>
											<tr>
												<td colspan="10">
											...
											<tr style="font-weight:bold; text-transform: uppercase;">
												<td colspan="8">Exchange (Refer sale bill no : <?php
											...
											foreach ($est_other_item['return_details'] as $items) {
												...
												if ($items['wastage_discount'] > 0) {
													...
													$wastge_wt = $wastge_wt - $discount_weight;
											...
											?>
												...
												<tr>
													...
													<td style="width: 42%;" class="textOverflowHidden"><?php echo $items['product_name']; ?></td>
													...
													<td style="width: 10%" class="alignRight"><?php echo moneyFormatIndia($items['rate_per_grm']); ?></td>
													<td style="width: 12%" class="alignRight"><?php echo moneyFormatIndia(number_format($item_cost, 2, '.', '')); ?></td>
												</tr>
												<?php $i++;
											...
											if (sizeof($est_other_item['old_matel_details']) > 0) {
												if ($total_sgst > 0) { ?>
													<tr>
														<td colspan="6"></td>
														<td class="alignRight">SGST <?php echo  $tax_percentage . '%'; ?><span style="margin-left: 10%;">&#8377;</span></td>
												...
												<?php if ($total_cgst > 0) { ?>
													<tr>
														<td colspan="6"></td>
														<td class="alignRight">CGST <?php echo  $tax_percentage . '%'; ?><span style="margin-left: 10%;">&#8377;</span></td>
														<td class="alignRight"><?php echo moneyFormatIndia(number_format($total_cgst, 2, '.', '')); ?></td>
													</tr>

												<?php } ?>
												<?php $return_round_off_amt = moneyFormatIndia(number_format($billing['return_round_off_amt'], 2, '.', ''));
												if ($return_round_off_amt != 0) { ?>
													<tr>
														<td colspan="6"></td>
														<td class="alignRight">Round Off<span style="margin-left: 10%;">&#8377;</span></td>
														<td class="alignRight"><?php echo moneyFormatIndia(number_format($return_round_off_amt, 2, '.', '')); ?></td>
													</tr>
												...
												<?php if ($total_return > 0) { ?>
													...
													<tr style="font-weight: bold">
														<td colspan="6"></td>
														<td class="alignRight">TOTAL<span style="margin-left: 10%;">&#8377;</span></td>
														<td class="alignRight"><?php echo moneyFormatIndia(number_format($total_return - $billing['credit_due_amt'] + $return_round_off_amt, 2, '.', '')); ?></td>
													</tr>
													<tr>
														<td colspan="9"></td>
														<td>
											...
											if ($pur_total_amt == 0) {
												if ($billing['credit_due_amt'] != 0) { ?>
													...
													<tr>
														<td colspan="6"></td>
														<td class="alignRight">Return Amount<span style="margin-left: 10%;">&#8377;</span></td>
														<td class="alignRight"><?php echo moneyFormatIndia(number_format($total_return, 2, '.', '')); ?></td>
													</tr>

													<tr>
														<td colspan="6"></td>
														<td class="alignRight">Credit Due Amt<span style="margin-left: 10%;">&#8377;</span></td>
												...
												<?php if ($tot_sales_amt + $billing['handling_charges'] > 0) { ?>
													<tr>
														<td colspan="6"></td>
														<td class="alignRight">Sales Amount<span style="margin-left: 10%;">&#8377;</span></td>
												...
												<?php if ($total_return > 0 && $billing['bill_type'] != 7) { ?>
													<tr>
														<td colspan="6"></td>
														<td class="alignRight">Exchange Amount<span style="margin-left: 10%;">&#8377;</span></td>
														<td class="alignRight"><?php echo moneyFormatIndia(number_format($total_return - $billing['credit_due_amt'] + $return_round_off_amt, 2, '.', '')); ?></td>
													</tr>
												<?php } ?>
												...
												<?php if ($tot_sales_amt > 0 && $total_return > 0) { ?>
													...
													<tr>
														<td colspan="6"></td>
														<td class="alignRight">Net Amount<span style="margin-left: 10%;">&#8377;</span></td>
									...
									<?php if (sizeof($est_other_item['sales_ret_trans_details']) > 0) { ?>
										...
										<div class="col-xs-12">
											<div class="table-responsive">
												<table id="pp" class="table text-center" style="table-layout: fixed;">
													<thead style="text-transform:uppercase;font-size:10px;">
														<tr>
											...
											<div class="table-responsive">
												<table id="pp" class="table text-center" style="table-layout: fixed;">
													...
													<tr style="text-transform:uppercase;font-size:12px;">
														...
														<th class="table_heading " style="width: 33%; text-align:left;">Description </th>
														<th class="table_heading alignRight" style="width: 10%;">HSN</th>
														...
														<th class="table_heading alignRight" style="width: 15%;">Amount</th>
													</tr>
													<tr>
														<td colspan="9">
															<hr class="old_metal_dashed">
													...
													?>
														<tr>
															...
															<td class='textOverflowHidden ' style="width: 33%;"><?php echo $items['product_name']; ?><?php echo " -" . $items['sub_design_name']; ?></td>
															<td class="alignRight" style="width: 10%;"><?php echo $items['hsn_code']; ?></td>
															<!-- <td><?php echo $items['purname']; ?></td> -->
															<td class="alignRight" style="width: 5%;"><?php echo $items['totalitems']; ?></td>
															<td class="alignRight" style="width: 10%;"><?php echo $items['weight'] . ($items['pro_uom'] != '' ? '-' . $items['pro_uom'] : ''); ?></td>
															<td class="alignRight" style="width: 10%;"><?php echo (!empty($items['net_wt']) ? $items['net_wt'] : '') . ($items['pro_uom'] != '' ? '-' . $items['pro_uom'] : ''); ?></td>
															<td class="alignRight" style="width: 12%;"><?php echo $items['rate_per_gram']; ?></td>
															<!-- <td class="alignRight"><?php echo moneyFormatIndia(number_format((float)($od_wastage_amt), 2, '.', '')); ?></td> -->
															<td class="alignRight" style="width: 15%;"><?php echo moneyFormatIndia(number_format(($rate_without_gst - $stone_amount), 2, '.', '')); ?></td>
														</tr>
														<?php
													...
													<tr class="total" style="font-weight: bold">
														...
														<td style="width: 10%;" class="alignRight"></td>
														<td style="width: 5%;" class="alignRight"><?php echo $od_pcs; ?></td>
														<td style="width: 10%;" class="alignRight"><?php echo number_format($weight, 3, '.', ''); ?></td>
														<td style="width: 10%;" class="alignRight"><?php echo number_format($od_net_wt, 3, '.', ''); ?></td>
														<td style="width: 12%;" class="alignRight"></td>
														<td style="width: 15%;" class="alignRight"><?php echo moneyFormatIndia(number_format((float)($od_total_price_without_gst), 2, '.', '')); ?></td>
													</tr>
													...
													<?php if ($od_total_price_without_gst > 0) { ?>
														<tr>
															<td colspan="6" class="alignRight">SUB TOTAL</td>
															<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
													...
													<?php if ($od_total_cgst > 0 || $od_total_sgst > 0 && $billing['is_eda'] == 1) { ?>
														...
														<tr>
															<td colspan="6" class="alignRight">CGST <?php echo (($items['tax_percentage'] ?? 0) / 2) . '%'; ?></td>
															<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
															<td class="alignRight"><?php echo moneyFormatIndia(number_format($od_total_cgst, 2, '.', '')); ?></td>
														</tr>

														<tr>
															<td colspan="6" class="alignRight">SGST <?php echo (($items['tax_percentage'] ?? 0) / 2) . '%'; ?></td>
															<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
															<td class="alignRight"><?php echo moneyFormatIndia(number_format($od_total_sgst, 2, '.', '')); ?></td>
														</tr>
													...
													<?php if ($od_total_igst > 0 && $billing['is_eda'] == 1) { ?>
														...
														<tr>
															<td colspan="6" class="alignRight">IGST</td>
															<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
															<td class="alignRight"><?php echo moneyFormatIndia(number_format($od_total_igst, 2, '.', '')); ?></td>
														</tr>
													...
													<?php if ($od_total_price > 0) { ?>
														...
														<tr class="table_heading">
															<td colspan="6" class="alignRight">Approx Total Amount</td>
															<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
							...
							<?php if (sizeof($est_other_item['sales_trasnfer_details']) > 0) { ?>
								...
								<div class="col-xs-12">
										...
										<table id="pp" class="table text-center">
											<thead style="text-transform:uppercase;font-size:10px; font-weight:bold;">
												<tr>
													...
													<td class="alignRight" width="12%;">Rate</td>
													<td class="alignRight" width="15%;" style="text-align:right;">Amount</td>
												</tr>
											</thead>
											<tr>
												<td colspan="7">
									...
									<tr>
										<td colspan="7" class="alignRight">Net Amount</td>
										<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
								...
								<?php } ?>
											...
											<?php if ($taxable_amt > 0) { ?>
												<tr>
													<td colspan="5" class="alignRight">SUB TOTAL</td>
													<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
											...
											<?php if ($total_sgst > 0) { ?>
												<tr>
													<td colspan="5" class="alignRight">SGST (<?php echo ($est_other_item['sales_trasnfer_details'][0]['tax_percentage'] / 2) . '%' ?>)</td>
													<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
													<td class="alignRight"><?php echo moneyFormatIndia(number_format($total_sgst, 2, '.', '')); ?></td>
												</tr>
											<?php } ?>
											...
											<?php if ($total_cgst > 0) { ?>
												<tr>
													<td colspan="5" class="alignRight">CGST (<?php echo ($est_other_item['sales_trasnfer_details'][0]['tax_percentage'] / 2) . '%' ?>)</td>
													<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
													<td class="alignRight"><?php echo moneyFormatIndia(number_format($total_cgst, 2, '.', '')); ?></td>
												</tr>
											<?php } ?>
											...
											<?php if ($total_igst > 0) { ?>
												<tr>
													<td colspan="5" class="alignRight">IGST (<?php echo ($est_other_item['sales_trasnfer_details'][0]['tax_percentage']) . '%' ?>)</td>
													<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
											...
											<?php if ($billing['round_off_amt'] > 0) { ?>
												<tr>
													<td colspan="5" class="alignRight">Round Off</td>
													<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
											...
											<?php if ($billing['handling_charges'] > 0) { ?>
												<tr>
													<td colspan="5" class="alignRight">H.C</td>
													<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
													<td class="alignRight"><?php echo moneyFormatIndia(number_format($billing['handling_charges'], 2, '.', '')); ?></td>
												</tr>
											<?php } ?>
											...
											<tr style="font-weight:bold;">
												...
												<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
												<td class="alignRight" style="font-size: 12px; "><?php echo moneyFormatIndia(number_format($billing['tot_bill_amount'], 2, '.', '')); ?></td>
											</tr>
						...
						<?php if (sizeof($est_other_item['old_matel_details']) > 0) { ?>
							...
							<table id="pp" class="table text-center">
								...
								<tr style="text-transform:uppercase;font-size:12px;">
									<th class="table_heading alignCenter" style="width: 5%; text-align:center;">S.No</th>
									<th colspan="2" class="table_heading textOverflowHidden" style="width: 25%; text-align:left;">Description </th>
									<th class="table_heading alignRight" style="width: 13%;">HSN CODE</th>
									<th class="table_heading alignRight" style="width: 8%;">GRSWT</th>
									<th class="table_heading alignRight" style="width: 10%;">MTL LESS</th>
									<th class="table_heading alignRight" style="width: 12%;">NETWT</th>
									<th class="table_heading alignRight" style="width: 12%;">RATE</th>
									<th class="table_heading alignRight" style="width: 15%;">Amount</th>
								</tr>
								...
								?>
									<tr>
										<td class="alignCenter" style="width: 5%"><?php echo $i; ?></td>
										<td style="font-size:10px; width:25%" colspan="2" class='textOverflowHidden'><?php echo $items['old_metal_type']; ?></td>
										<td style="width: 13%" class="alignRight"><?php echo $items['metal_type'] == 1 ? '7108000' : ($items['metal_type'] == 2 ? '71069100' : '1') ?></td>
										<td style="width: 8%" class="alignRight"><?php echo number_format($items['gross_wt'], 3, '.', ''); ?></td>
										...
										<td style="width: 12%" class="alignRight"><?php echo number_format($items['net_wt'], 3, '.', ''); ?></td>
								...
								?>
										<tr class="stones">
											<td></td>
											<?php if ($stoneItems['stone_cal_type'] == 1) { ?>
												<td colspan="6" class='textOverflowHidden stoneData'><?php echo $stoneItems['pieces'] . ' Pcs ' . $stoneItems['stone_name'] . ($stoneItems['price'] > 0 ? ' ' . number_format($stoneItems['wt'], 3, '.', '') . ' ' . $stoneItems['uom_short_code'] . ' x ' . moneyFormatIndia(number_format((float)($stoneItems['rate_per_gram']), 2, '.', '')) . '=' . moneyFormatIndia(number_format((float)($stoneItems['price']), 2, '.', '')) : ''); ?></td>
											<?php } else if ($stoneItems['stone_cal_type'] == 2) { ?>
												<td colspan="6" class='textOverflowHidden stoneData'><?php echo number_format($stoneItems['wt'], 3, '.', '') . ' ' . $stoneItems['uom_short_code'] . ' ' . $stoneItems['stone_name'] . ($stoneItems['price'] > 0 ? ' ' . $stoneItems['pieces'] . 'Pcs ' . 'x' . moneyFormatIndia(number_format((float)$stoneItems['rate_per_gram'], 2, '.', '')) . '=' . moneyFormatIndia(number_format((float)($stoneItems['price']), 2, '.', '')) : ''); ?></td>
											<?php } ?>
											<td class="alignRight stoneData"></td>
											<td class="alignRight stoneData"></td>
										</tr>
								...
								<tr style="font-weight: bold">
									<td class="alignCenter" style="width: 5%; font-size: 12px;">TOTAL</td>
									<td colspan="2" style="width: 25%;"></td>
									<td class="alignRight" style="width: 13%;"></td>
									<td class="alignRight" style="font-size: 10px;"><?php echo number_format($gross_wt, 3, '.', ''); ?></td>
									<td class="alignRight" style="font-size: 10px;"></td>
									<td class="alignRight" style="font-size: 10px;"><?php echo number_format($net_wt, 3, '.', ''); ?></td>
									<td class="alignRight" style="font-size: 10px;"></td>
									<td class="alignRight" style="font-size: 10px;"><?php echo moneyFormatIndia(number_format((float)$pur_total_amt, 2, '.', '')); ?></td>
								</tr>
								<tr>
									<td colspan="9">
										<hr class="old_metal_dashed">
								...
								<?php if ($tot_sales_amt + $billing['handling_charges'] > 0) { ?>
									<tr style="font-weight: bold">
										<td colspan="7" class="alignRight">Sales Amount</td>
										<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
								...
								<?php if ($total_return > 0 && $billing['bill_type'] != 7) { ?>
									<tr>
										<td colspan="7" class="alignRight">Return Amount</td>
										<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
										<td class="alignRight"><?php echo moneyFormatIndia(number_format($total_return + $return_round_off_amt, 2, '.', '')); ?></td>
									</tr>
								<?php } ?>
								<?php if ($pur_total_amt > 0) { ?>
									<tr style="font-weight:bold;">
										<td colspan="7" class="alignRight">Purchase Amount</td>
										<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
										<td class="alignRight"><?php echo moneyFormatIndia(number_format($pur_total_amt, 2, '.', '')); ?></td>
									</tr>
								<?php } ?>
								...
								<?php if ($tot_sales_amt > 0) { ?>
									...
									<tr style="font-weight: bold">
										...
										<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
										<td class="alignRight"><?php echo moneyFormatIndia(number_format(($tot_sales_amt + $billing['handling_charges']) - $total_return - $pur_total_amt + $round_off - $return_round_off_amt, 2, '.', '')); ?></td>
									</tr>
									<tr>
										<td colspan="8"></td>
										<td>
						...
						<table id="pp" style="width:100%; border-collapse:collapse; table-layout: fixed; font-size:10px;">
							...
							if (sizeof($get_other_adv_details) > 0) { ?>
								<tr style="font-weight: bold;">
									<td colspan="6" class="alignRight">Order Amount</td>
									<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
									<td class="alignRight"><?php echo moneyFormatIndia(number_format($od_total_price, 2, '.', '')); ?></td>
								</tr>
								<?php }
							...
							if (sizeof($get_other_adv_details) > 0) { ?>
								<!-- <tr>
															...
															<tr style="font-weight: bold;">
																...
																<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
																<td class="alignRight"><?php echo moneyFormatIndia(number_format($balance_metal, 3, '.', '')); ?></td>
															</tr> -->
							...
							<?php if ($tot_adv_amt > 0) { ?>
								<tr style="font-weight: bold;">
									<td colspan="6" class="alignRight"> Advance Amount</td>
									<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
					...
					<?php
					if ($cash_amt > 0 || $ord_adj_amt > 0 || $chit_adj > 0 || $chq_amt > 0 || $card_amt > 0 || $upi_amt > 0 || $rtgs_amt > 0 || $imps_amt > 0 || $adv_adj > 0 || $gift_amount > 0 || $billing['advance_deposit'] > 0 || $neft_amt > 0 || $due_amount > 0 || $adj_amt > 0 || $billing['adv_adj_amt'] > 0) { ?>
						<table id="pp" class="payment-mode-details" style="width:100%; border-collapse:collapse; table-layout: fixed; font-size:12px; font-weight: bold; color: #000 !important;">
							<thead style="text-transform:uppercase; visibility:hidden;">
								<tr>
									<th style="width: 5%;"></th>
									<th style="width: 33%;"></th>
									...
									<th style="width: 10%;"></th>
									<th style="width: 10%;"></th>
									<th style="border:none; width: 12%;"></th>
							...
							<?php if ($chit_adj != 0 && ($billing['bill_type'] != 10)) { ?>
								<tr>
									<td colspan="6" class="alignRight" style="font-weight: bold;">CHIT ADJ</td>
									<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
									<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format(round($chit_adj + $scheme_benefit), 2, '.', '')); ?></td>
								</tr>
								<tr>
									<td colspan="8"></td>
									<td>
								...
								<tr>
									<td colspan="6" class="alignRight" style="font-weight: bold;">Balance Amount</td>
									<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
									<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format(round($tot_sales_amt + $billing['handling_charges'] - $pur_total_amt - $total_return + $round_off - ($chit_adj + $scheme_benefit)), 2, '.', '')); ?></td>
								</tr>
							<?php } ?>
							...
							<?php } ?> -->
							<?php if ($billing['adv_adj_amt'] > 0) { ?>
								<tr>
									<td colspan="6" class="alignRight">ADVANCE ADJ</td>
									<td class="alignRight" style="font-size:15px !important; border:none;">&#8377;</td>
									<td class="alignRight" style="border:none;"><?php echo moneyFormatIndia(number_format($billing['adv_adj_amt'], 2, '.', '')); ?></td>
								</tr>
							<?php } ?>
							<?php if ($chq_amt > 0) { ?>
								<?php foreach ($payment['pay_details'] as $chq) {
									if ($chq['payment_mode'] == 'CHQ') { ?>
										<tr>
											<td colspan="5" class="alignLeft"><?php echo 'Chq No: <strong>' . $chq['cheque_no'] . '</strong> / <strong>' . $chq['payment_ref_number'] . '</strong> Date: <strong>' . $chq['cheque_date'] . '</strong>'; ?></td>
											<td class="alignRight" style="font-weight: bold;">CHEQUE</td>
											<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
											<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format($chq['payment_amount'], 2, '.', '')); ?></td>
										</tr>
							<?php }
								}
							} ?>
								...
								<?php foreach ($payment['pay_details'] as $cardItem) {
									if ($cardItem['payment_mode'] == 'DC' || $cardItem['payment_mode'] == 'CC') { ?>
										<tr>
											...
											<td class="alignRight" style="font-weight: bold;">CARD</td>
											<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
											<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format($cardItem['payment_amount'], 2, '.', '')); ?></td>
										</tr>
							<?php }
								}
							} ?>
								...
								<?php foreach ($payment['pay_details'] as $rtgs) {
									if ($rtgs['payment_mode'] == 'NB' && $rtgs['transfer_type'] == 'RTGS') { ?>
										<tr>
											...
											<td class="alignRight" style="font-weight: bold;">RTGS</td>
											<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
											<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format($rtgs['payment_amount'], 2, '.', '')); ?></td>
										</tr>
							<?php }
								}
							} ?>
								...
								<?php foreach ($payment['pay_details'] as $imps) {
									if ($imps['payment_mode'] == 'NB' && $imps['transfer_type'] == 'IMPS') { ?>
										<tr>
											...
											<td class="alignRight" style="font-weight: bold;">IMPS</td>
											<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
											<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format($imps['payment_amount'], 2, '.', '')); ?></td>
										</tr>
							<?php }
								}
							} ?>
								...
								<?php foreach ($payment['pay_details'] as $neft) {
									if ($neft['payment_mode'] == 'NB' && $neft['transfer_type'] == 'NEFT') { ?>
										<tr>
											...
											<td class="alignRight" style="font-weight: bold;">NEFT</td>
											<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
											<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format($neft['payment_amount'], 2, '.', '')); ?></td>
										</tr>
							<?php }
								}
							} ?>
								...
								<?php foreach ($payment['pay_details'] as $upi) {
									if ($upi['payment_mode'] == 'NB' && $upi['transfer_type'] == 'UPI') { ?>
										<tr>
											...
											<td class="alignRight" style="font-weight: bold;">UPI</td>
											<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
											<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format($upi['payment_amount'], 2, '.', '')); ?></td>
										</tr>
							<?php }
								}
							} ?>
							...
							<?php if ($due_amount > 0) { ?>
								<tr>
									<td colspan="6" class="alignRight" style="font-weight: bold;">DUE AMOUNT<?php echo " (" . $billing['credit_due_date'] . ")" ?></td>
									<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
									<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format($due_amount, 2, '.', '')); ?></td>
								</tr>
							<?php } ?>
							...
							<?php if ($adv_adj > 0) { ?>
								<tr>
									<td colspan="6" class="alignRight">ADVANCE ADJ</td>
									<td class="alignRight" style="font-size:15px !important; border:none;">&#8377;</td>
									<td class="alignRight" style="border:none;"><?php echo moneyFormatIndia(number_format($adv_adj, 2, '.', '')); ?></td>
								</tr>
							<?php } ?>
							...
							<?php if ($bouns_amt != 0) { ?>
								<tr>
									<td colspan="6" class="alignRight">BONUS</td>
									<td class="alignRight" style="font-size:15px !important; border:none;">&#8377;</td>
									<td class="alignRight" style="border:none;"><?php echo moneyFormatIndia(number_format($bouns_amt, 2, '.', '')); ?></td>
								</tr>
							<?php } ?>
							...
							<?php if ($gift_amount != 0) { ?>
								<tr>
									<td colspan="6" class="alignRight">GIFT UTILIZED</td>
									<td class="alignRight" style="font-size:15px !important; border:none;">&#8377;</td>
									<td class="alignRight" style="border:none;"><?php echo moneyFormatIndia(number_format($gift_amount, 2, '.', '')); ?></td>
								</tr>
							<?php } ?>

							<?php if ($billing['credit_disc_amt'] != 0) { ?>
								<tr>
									<td colspan="6" class="alignRight">DISCOUNT</td>
									<td class="alignRight" style="font-size:15px !important; border:none;">&#8377;</td>
									<td class="alignRight" style="border:none;"><?php echo moneyFormatIndia(number_format($billing['credit_disc_amt'], 2, '.', '')); ?></td>
								</tr>
							<?php } ?>

							<?php if ($billing['advance_deposit'] != 0) { ?>
								<tr>
									<td colspan="6" class="alignRight">Advance</td>
									<td class="alignRight" style="font-size:15px !important; border:none;">&#8377;</td>
									<td class="alignRight" style="border:none;"><?php echo moneyFormatIndia(number_format($billing['advance_deposit'], 2, '.', '')) ?></td>
								</tr>
							<?php } ?>

							<?php if ($cash_amt > 0) { ?>
								<tr>
									<td colspan="6" class="alignRight" style="font-weight: bold;">Cash Received</td>
									<td class="alignRight" style="font-size:15px !important; border:none; font-weight: bold;">&#8377;</td>
									<td class="alignRight" style="border:none; font-weight: bold;"><?php echo moneyFormatIndia(number_format($cash_amt, 2, '.', '')); ?></td>
								</tr>
							<?php } ?>
							<?php if ($ord_adj_amt > 0) { ?>
								<tr>
									<td colspan="6" class="alignRight">Advance Amount</td>
									<td class="alignRight" style="font-size:15px !important; border:none;">&#8377;</td>
									<td class="alignRight" style="border:none;"><?php echo moneyFormatIndia(number_format($ord_adj_amt, 2, '.', '')); ?></td>
								</tr>
							<?php } ?>
		</table>
	...
	<?php
	if ($cash_amt < 0 || $chq_amt < 0  || $rtgs_amt < 0 || $imps_amt < 0 || $upi_amt < 0 || $card_amt < 0) {


?>

		<table id="pp" class="pay_mode_totals table text-center payment-mode-details" style="width:100%; border-collapse:collapse; table-layout: fixed; font-size:12px; font-weight: bold; color: #000 !important;">
			<thead style="text-transform:uppercase; visibility:hidden;">
				<tr>
					<th style="width: 5%;"></th>
					<th style="width: 33%;"></th>
			...
			<?php if ($chq_amt < 0) { ?>
				<?php foreach ($payment['pay_details'] as $chq) {
					if ($chq['payment_mode'] == 'CHQ') { ?>
						<tr>
							<td colspan="5"> <?php echo 'Chq No: ' . $chq['cheque_no'] . '/  ' . $chq['payment_ref_number'] . 'Date: ' . $chq['cheque_date']; ?></td>
			...
			<?php if ($cash_amt < 0) { ?>
				<tr>
					<td colspan="5"></td>
					<th colspan="1" class="alignRight">Cash Paid</th>
					<th class="alignRight" style="font-size:15px !important;">&#8377;</th>
	...
	if (sizeof($get_other_adv_details) > 0 || $od_total_price > 0) {
		...
		$first_rate = $rate_new;
		$od_balance = $balance_amt - ($total_amt + $billing['adv_adj_amt']); // Adjusted balance amount by adding general advance adjustment
	...
	?>
		<table id="pp" style="width:100%; border-collapse:collapse; table-layout: fixed; font-size:10px;">
			...
			<?php if ($balance_type == 1) { ?>
				<tr style="font-weight: bold;">
					<td colspan="6" class="alignRight">Balance Wt (<?php echo moneyFormatIndia(number_format($od_balance, 2, '.', '')) . ' / ' . moneyFormatIndia(number_format($first_rate, 2, '.', '')); ?>)</td>
					<td class="alignRight" style="font-size:15px !important;"></td>
					<td class="alignRight"><?php echo moneyFormatIndia(number_format($od_metal_balance, 3, '.', '')) . 'Gm'; ?></td>
				</tr>
			<?php } else { ?>
				<tr style="font-weight: bold;">
					<td colspan="6" class="alignRight">Approx Balance Amount</td>
					<td class="alignRight" style="font-size:15px !important;">&#8377;</td>
	...
	<?php if ((sizeof($est_other_item['chit_details']) > 0) && $billing['bill_type'] != 10) { ?>
		<div class="col-xs-6" style="width: 60%;">
			<div class="table-responsive">
				<table id="pp" class="table text-center" style="width: 100%; table-layout: fixed;">
					<colgroup>
						<col style="width: 10%;">
						<col style="width: 23%;">
						<col style="width: 21%;">
					...
					<tr>
						<td colspan="5">
							<hr class="item_dashed" style="width: 100%;">
						</td>
					</tr>
					<tr style="text-transform:uppercase;font-size:12px;">
						...
						<th style="text-align: right;">Scheme Disc</th>
					...
					$i = 1;
					$sheme_close = 0;
					$tot_scheme_benefit = 0;
					$grand_total = 0;
					$total_apx_benefit = 0;

					$is_split_bill = 0;
					foreach ($est_other_item['item_details'] as $items) {
						if($items['isTagsplitted'] == 1){
							$is_split_bill = 1;
						}
					}

					foreach ($est_other_item['chit_details'] as $chit) { 
						// Calculate scheme benefit for each chit
						$scheme_benefit = 0;
						if ($chit['closing_weight'] > 0) {
							$rate_per_gram = ($chit['rate_per_gram'] > 0 ? $chit['rate_per_gram'] : $billing['goldrate_22ct']);
							$closing_weight = number_format($chit['closing_weight'], 3, '.', '');
							...
							// Base calculation: (closing_weight * rate_per_gram) - closing_amount + additional_benefits
							$base_benefit = ($closing_weight * $rate_per_gram) - $closing_amount + $chit['additional_benefits'];
							
							// Only include savings_in_making_charge and savings_in_wastage if NOT a split bill
							if ($is_split_bill == 0) {
								$savings_in_making_charge = $chit['savings_in_making_charge'];
								$savings_in_wastage = ($chit['savings_in_wastage'] * $rate_per_gram);
								$base_benefit += $savings_in_making_charge + $savings_in_wastage;
							} else {
								// For split bill, accumulate savings separately for Apx Scheme Benefit
								$savings_in_making_charge = $chit['savings_in_making_charge'];
								$savings_in_wastage = ($chit['savings_in_wastage'] * $rate_per_gram);
								$total_apx_benefit += $savings_in_making_charge + $savings_in_wastage;
							}
							
							$scheme_benefit = number_format($base_benefit, 0, '.', '');
						}
						...
						<tr>
							...
							<td style="text-align: left;"><?php echo $chit['scheme_acc_number']; ?></td>
							<td style="text-align: right;"><?php echo moneyFormatIndia(number_format($payable_amount, 2, '.', '')); ?></td>
							<td style="text-align: right;"><?php echo moneyFormatIndia(number_format($scheme_benefit, 2, '.', '')); ?></td>
							<td style="text-align: right;"><?php echo moneyFormatIndia(number_format($row_total, 2, '.', '')); ?></td>
						</tr>
					<?php 
						$i++;
						$sheme_close += $payable_amount;
						$tot_scheme_benefit += $scheme_benefit;
					...
					<tr style="font-weight: bold;">
						<td></td>
						<td></td>
						<td style="text-align: right;"><?php echo moneyFormatIndia(number_format($sheme_close, 2, '.', '')); ?></td>
						<td style="text-align: right;"><?php echo moneyFormatIndia(number_format($tot_scheme_benefit, 2, '.', '')); ?></td>
						<td style="text-align: right;"><?php echo moneyFormatIndia(number_format($grand_total, 2, '.', '')); ?></td>
					</tr>
					<tr>
						<td colspan="5">
							<hr class="item_dashed" style="width: 100%;">
					...
					<?php if ($is_split_bill == 1 && $total_apx_benefit > 0) { ?>
					<tr>
						<td colspan="2" style="font-weight: bold; text-align: left;">Apx Scheme Discount</td>
						<td style="font-weight: bold; text-align: right;"><?php echo moneyFormatIndia(number_format($total_apx_benefit, 2, '.', '')); ?></td>
						<td colspan="2"></td>
					</tr>
					<?php } ?>
				</table>
	...
	<?php if ($billing['gift_issue_amount'] > 0) { ?>
		<span>Gift Voucher Worth<span style="margin-left: 10%;">&#8377;</span><?php echo ' ' . $billing['gift_issue_amount'] . ' ' ?>Valid Till <?php echo $billing['valid_to'] . '. Voucher Code - ' . $billing['code'] . ''; ?></span>
	...
	<?php if (sizeof($receiptDetails) > 0) {
		...
		foreach ($receiptDetails as $val) { ?>
			...
			<!-- <div style="margin-top: 3px; margin-bottom: 7px">
				...
				<table id="pp" class="table text-center" style="width:80%;">
					<tr>
					...
					<tbody>
						<tr>
							...
							<td><?php echo number_format($val['refund_amt'], 2, '.', ''); ?></td>
	...
	<div class="footer">
		...
		<!-- <table style="width:100%; border-collapse:collapse; font-weight:bold; margin-top:20px;">
			<tr>
				<td style="font-size:16px; text-align:left; width:50%; padding:10px 0; border:none;">Customer Signature</td>
				<td style="font-size:16px; text-align:right; width:50%; padding:10px 0; border:none;">For <?php echo $comp_details['company_name']; ?></td>
			</tr>
		</table> -->
