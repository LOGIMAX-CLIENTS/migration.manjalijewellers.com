		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<div class="form-group pull-right">
					<input name="scheme[sch_approval]" type="hidden" id="sch_approval">
				</div>
				<div class="col-md-10 col-md-offset-1">
					...
					<input type="hidden" id="total_paid_ins" value="<?php echo $scheme['total_paid_ins']; ?> ">
					<input type="hidden" id="is_topup_scheme" value="<?php echo $scheme['is_topup_scheme']; ?>">
						...
						<?php if ($this->session->userdata('branch_settings') == 1) { ?>
							<div class="col-sm-4">
								<div class="form-group" style="height:60px">
									...
									<select required id="branch_select" <?php echo ($scheme['total_paid_ins'] > 0 && !empty($scheme['id_branch'])) ? 'disabled' : ''; ?> class="form-control"></select>
									<!-- esakki 29-11 -->
									<input id="id_branch" name="scheme[id_branch]" type="hidden" value="<?php echo set_value('scheme[id_branch]', $scheme['id_branch']); ?>" />
								</div>
							</div>
						<?php  } else {  ?>
							<input type="hidden" id="id_branch" name="scheme[id_branch]" value="<?php echo $this->session->userdata('id_branch'); ?>">
						<?php  }  ?>
						<div class="col-sm-4">
							<div class="form-group" style="height:60px">
								...
								<input type="text" required="true" class="form-control mobile_number" <?php echo (!empty($scheme['account_name'])) ? 'disabled' : ''; ?> name="mobile_number" placeholder="Enter The Mobile Number" id="mobile_number" value="<?php echo htmlspecialchars((!empty($scheme['account_name']) ? $scheme['account_name'] . (!empty($scheme['mobile']) ? ' - ' . $scheme['mobile'] : '') : $scheme['mobile'])); ?>" style="width: 99%;" autocomplete="off">
								<!--              				    <input type="text" required="true" class="form-control mobile_number" name="mobile_number" placeholder="Enter The Mobile Number" id="mobile_number" value="<?php echo htmlspecialchars(set_value('scheme[mobile]', (isset($scheme['account_name']) ? $scheme['account_name'] : '') . ' - ' . (isset($scheme['mobile']) ? $scheme['mobile'] : ''))); ?>" style="width: 99%;" autocomplete="off">
-->
								...
								<input type="hidden" id="session_cus_mobile" value="<?php echo $this->session->userdata('cus_mobile'); ?>" /> <!-- esakki 11-11 -->
								<input type="hidden" id="session_cus_name" value="<?php echo $this->session->userdata('cus_name'); ?>" /> <!-- esakki 11-11 -->
								<input type="hidden" id="session_cus_id" value="<?php echo $this->session->userdata('cus_id'); ?>" /> <!-- esakki 11-11 -->
								<input type="hidden" id="branchwise_scheme" value="<?php echo $this->session->userdata('branchwise_scheme'); ?>" />
								<input type="hidden" id="login_branch" value="<?php echo $this->session->userdata('login_branch'); ?>" />
							</div>
							<?php
								...
								<div class="box box-default collapsed-box">
									...
									<div class="box-body">
										<div class="row">
											<div class="col-sm-4">
												<div class="form-group">
													<img id="cus_img" src="<?php echo base_url((isset($scheme['cus_img']) ? $customer['cus_img'] : 'assets/img/default.png')); ?>"
												class="img-thumbnail" style="width:175px;height:100%;"
												alt="Customer image">
											<input type="hidden" name="scheme[customer_img]"
												value="<?php echo set_value('scheme[customer_img]', $scheme['cus_img']) ?>" />
											...
											<div class="col-sm-8">
												<div class="form-group">
													<label>Address</label>
													<label id="cus_address"> : <?php echo  $scheme['address1'] ?> </label>
												</div>
												<div class="form-group">
													<label>Mobile</label>
													<label id="cus_mobile">: <?php echo  $scheme['mobile'] ?></label>
												</div>
												<div class="form-group">
													<label>Phone</label>
													<label id="phone_mobile">: <?php echo  $scheme['phone'] ?> </label>
												</div>
											</div>
										</div>
									</div><!-- /.box-body -->
						...
						<div class="col-sm-4">
							<div class="form-group">
								...
								<select required class="form-control" id="scheme" name="scheme[id_scheme]" <?php echo ($scheme['total_paid_ins'] > 0 && !empty($scheme['id_scheme'])) ? 'disabled' : ''; ?>></select>
								<input type="hidden" id="hid_id_scheme" name="scheme_val" value="<?php echo set_value('scheme_val', $scheme['id_scheme']); ?>" />
							</div>
							<div class="form-group">
								<div class="box box-default collapsed-box">
									<div class="box-header with-border">
						...
						<div class="col-md-2" id="active_check">
							<div class="form-group pull-right">
								<label>Active</label>
								<input type="checkbox" id="active" class="switch" data-on-text="YES" data-off-text="NO" name="scheme[active]" value="1" <?php if ($scheme['active'] == 1) { ?> checked="true" <?php } ?> />
							</div>
							...
							<div class="form-group">
								<div class="col-md-<?php echo ($scheme['id_scheme_account'] != NULL ? '3' : '3'); ?>">
									<div class="form-group">
										...
										<input type="hidden" id="cusName_edit" name="cusName_edit" value="<?php echo set_value('cusName_edit', $entry_date[0]['cusName_edit']); ?>"> <!-- esakki 11-11 -->
										<?php if ($entry_date[0]['cusName_edit'] == 1) { ?> <!-- A/c name edit option in admin based on the settings HH-->
											<input type="text" oninput="this.value = this.value.toUpperCase()" class="form-control" id="account_name" name="scheme[account_name]" required="true" value="<?php echo set_value('scheme[account_name]', $scheme['account_name']); ?>" />
										<?php } else if ($scheme['id_scheme_account'] == '' && $entry_date[0]['cusName_edit'] == 0) { ?>
											<input type="text" oninput="this.value = this.value.toUpperCase()" class="form-control" id="cus_name" name="scheme[cus_name]" value="<?php echo set_value('scheme[cus_name]', $scheme['cus_name']); ?>" readonly="true" />
										<?php } else if ($scheme['id_scheme_account'] != '' && $entry_date[0]['cusName_edit'] == 0) { ?>
											<input type="text" oninput="this.value = this.value.toUpperCase()" class="form-control" id="account_name" name="scheme[account_name]" required="true" value="<?php echo set_value('scheme[account_name]', $scheme['account_name']); ?>" readonly="true" />
										<?php } ?>
									</div>
								</div>
								<!--<input value="<?php echo  $scheme['referal_code'] ?>"> -->
								<div class="col-md-3" class="referalcode">
									<div class="form-group" id="referalcode" style="display:block">
										<label for=""><a data-toggle="tooltip" title="">Refferal By</a><span class="error"><span></label><br>
										<input type="radio" name="scheme[referal_by]" id="emp_referal_by" value="1" <?php echo ($scheme['is_refferal_by'] == 1) ? 'checked' : ''; ?>> Employee &nbsp;&nbsp;
										<input type="radio" name="scheme[referal_by]" id="cus_referal_by" value="0" <?php echo ($scheme['is_refferal_by'] == 0) ? 'checked' : ''; ?>> Customer &nbsp;&nbsp;
										<input type='hidden' class='form-control width50' id='referal_by' value="<?php echo set_value('scheme[is_refferal_by]', $scheme['is_refferal_by']); ?>" />
									</div>
								</div>
								<!-- <?php //echo '<pre>';print_r($scheme);exit; ?> -->
								<div class="col-md-3" class="referalcode" id="ref_code_input">
									<div class="form-group" id="referalcode" style="display:block">
										...
										<input type="text" class="form-control" name="scheme[referal_code]" placeholder="Enter The Referral code" id="referal_code" value="<?php echo set_value('scheme[referal_code]', $scheme['referal_code']); ?>" style="width: 99%;" autocomplete="off">
										<div style="color:red;font-size:15px;">
											<span id="ref_name"></span>
										</div>
								...
								<!-- <div class="col-md-3">
									<div class="form-group" id="referalcode" style="display:block">
										<label for=""><a data-toggle="tooltip" title="Enter the valid Referral code Ex Customer  mobile or Employee Emp-1 etc ">Refferal Code</a><span class="error"><span></label>
										<input type='number' class='form-control width50' name="scheme[referal_code]" <?php echo ($scheme['total_paid_ins'] > 0 && !empty($scheme['referal_code'])) ? 'disabled' : ''; ?> placeholder='Refferal Code' id='referal_code' value="<?php echo set_value('scheme[referal_code]', $scheme['referal_code']); ?>" />
										<div style="color:red;font-size:15px;">
											<span id="ref_name">
											</span>
										</div>
								...
								<div class="col-md-3" style="margin-bottom: 25px;">
									<div class="form-group" id="referalcode" style="display:block">
										...
										<input type='number' class='form-control width50' name="scheme[agent_code]" <?php echo ($scheme['total_paid_ins'] > 0 && !empty($scheme['agent_code'])) ? 'disabled' : ''; ?> placeholder='Agent Code' id='agent_code'
								...
								<div class="col-md-3" id="get_amt" style="display: none;">
									<div class="form-group">
										...
										<input type='number' class='form-control width50' name="scheme[firstPayment_amt]" placeholder='Payment Amount' <?php echo ($scheme['total_paid_ins'] > 0 && !empty($scheme['firstPayment_amt'])) ? 'disabled' : ''; ?> id='firstPayment_amt' value="<?php echo set_value('scheme[firstPayment_amt]', $scheme['firstPayment_amt']); ?>" />
								...
								<div class="col-md-3">
									<?php if ($this->session->userdata('branch_settings') == 1 && $this->session->userdata('id_branch') == '') { ?>
										<div class="form-group" align="center">
											...
											<select required <?php echo ($scheme['total_paid_ins'] > 0 && !empty($scheme['id_employee'])) ? 'disabled' : ''; ?> id="employee_select" <?php echo ($scheme['total_paid_ins'] > 0 && !empty($scheme['id_employee'])) ? 'disabled' : ''; ?> class="form-control" required='true'></select>
											<input id="id_employee" name="scheme[id_employee]" type="hidden" value="<?php echo $scheme['id_employee'] ?>">
										</div>
									<?php } ?>
								</div>
								<div class="col-md-3" id="lumpwgt_slabs_block" style="display: none;">
									<div class="form-group">
										<label for=""><a data-toggle="tooltip" title="Select the overall weight ">Select Weight (In grams)</a><span class="error">*<span></label>
										<select id="lumpwgt_slabs" class="form-control"></select>
										<input id="lump_joined_weight" name="scheme[lump_joined_weight]" type="hidden" value="<?php echo set_value('scheme[lump_joined_weight]', $scheme['lump_joined_weight']); ?>" />
										<input id="is_lumpSum" name="scheme[is_lumpSum]" type="hidden" value="" />
									</div>
								</div>
...
<div class="row">
   ...
   <div class="col-sm-3">
	   ...
	   <div class="form-group">
		   ...
		   <?php ($scheme['last_paid_date']);
			if (($this->config->item('showToAdminsOnly') == 1 && $this->session->userdata('profile') <= 2) || $this->config->item('showToAdminsOnly') == 0) { ?> 
		   <div class="input-group date">
		   ...
		   <div class="col-sm-4">
			 ...
			 <p class="help-block">No.of payments made in last month</p>      			            				            
				...
				<?php } ?>
					...
					<!--Duplicate passbook  -->
					<!-- <legend style="display:none"> Duplicate passbook</legend>
	   <div class="row" style="display:none">
		  <div class="col-md-12"> 
				 <div class="form-group">
					 <label>Duplicate passbook issued</label>
					 <input type="checkbox"  id="duplicate_passbook_issued" class="switch" data-on-text="YES" data-off-text="NO" name="scheme[duplicate_passbook_issued]" value="1" <?php if ($scheme['duplicate_passbook_issued'] == 1) { ?> checked="true" <?php } ?>/>
				 </div>
		 </div>	
	 </div>	 -->
					<!--Duplicate passbook -->
			 ...
			 <div class="col-md-6" id="show_prizes">
			  <div class="form-group">
				...
				<textarea  type="text" class="form-control" placeholder="Enter gift/prize issue description" name="gift[1][prize]" ></textarea>
					...
					<div class="row" id="topup_sch" style="display:none;">
						<legend>Topup</legend>
						<div class="col-md-12">
							<div class="col-md-3">
								<div class="form-group">
									<label for="">Booking Weight<span class="error">*</span></label>
									<input type='any' class='form-control width50' name="scheme[topup_weight]" id='topup_weight' value="<?php echo set_value('scheme[topup_weight]', $scheme['topup_weight']); ?>" />
								</div>
							</div>
							<div class="col-md-3">
								<div class="form-group">
									<label for="">Booking Amount<span class="error">*</span></label>
									<input type='any' class='form-control width50' name="scheme[topup_amount]" id='topup_amount' value="<?php echo set_value('scheme[topup_amount]', $scheme['topup_amount']); ?>" />
								</div>
							</div>
							<div class="col-md-3">
								<div class="form-group">
									<label for="">Booking Rate<span class="error">*</span></label>
									<input type='text' class='form-control width50' name="scheme[topup_rate]" id='topup_rate' value="<?php echo set_value('scheme[topup_rate]', $scheme['topup_rate']); ?>" readonly />
									<input type="hidden" id="topup_booking_type" value="<?php echo $scheme['topup_booking_type']; ?>">
									<input type="hidden" id="topup_min_value" value="<?php echo $scheme['topup_min_value']; ?>">
									<input type="hidden" id="topup_max_value" value="<?php echo $scheme['topup_max_value']; ?>">
									<input type="hidden" id="topup_denomination" value="<?php echo $scheme['topup_denomination']; ?>">
								</div>
							</div>
							<!--<div class="col-md-6" style="color:red;font-size:15px;">
								<span id="topup_msg"></span>
							</div>-->
						</div>
					</div>
					<legend> Gift Articles</legend>
											...
											<table id="chart_gift_creation_tbl" width="50%" cellpadding="0"
												...
												<thead>
													<tr>
														...
														<th>Bar Code<span class="error">*</span></th>
														<th>Action</th>
													</tr>
												</thead>
												<tbody>
														...
														foreach ($gift_data as $key => $gift) {
															...
															if ($gift['type'] == 'GIFT') { ?>
																<tr rowID="<?php echo $key + 1 ?>" id="<?php echo $key + 1 ?>">
																	<td>
																		<select style='width:200px;' required name='gifts' id='gift_list_<?php echo $key + 1 ?>' name='gift_select[<?php echo $key + 1 ?>]'
																	...
																	<td>
																		<input style='width:80px;' required type='number' name='gift_quantity[<?php echo $key + 1; ?>]' id='gift_quantity<?php echo $key + 1; ?>' value='<?php echo $gift['quantity'];  ?>' oninput='check_quantity_limit(this.value,<?php echo $key + 1 ?>)' /><br /><span class='error' id='limit_msg<?php echo $key + 1 ?>'>Qty Limit :<?php echo $gift['quantity_limit'] - $gift['out_stock']; ?></span>
																	</td>
																	<td>
																		<input style='width:110px;' type='text' name='gift_barcode[<?php echo $key + 1; ?>]' id='gift_barcode<?php echo $key + 1; ?>' value='<?php echo $gift['barcode'];  ?>' /><br />
																	</td>
																	<td>
																		<div>
					...
					<div class="col-sm-12">
						<div class="box box-default"><br />
							<div class="col-xs-offset-5">
								<input id="scheme_join_otp" type="hidden">
								<input id="scheme_join_otp_valid" type="hidden" value="0">
...
<!--modal for scheme join otp   ---- otp-- Starts here-->
<!-- modal -->
<div class="modal fade" id="verify_otp_scheme_join" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
			...
			<div class="row">
				<div class="modal-body text-center">
					<label class="col-md-5 text-right">Mobile Number :</label>
					<input class="col-md-4" id="txt_mob" readonly type="text" value="" />
				</div>
			</div>
			<div class="row" id="otp_txt_boxs">
				<div class="modal-body text-center">
					<label class="col-md-5 text-right">Enter OTP :</label>
					<input class="col-md-4" id="otp_scheme" type="text" value="" />
				</div>
			</div>
			<br>
			<br>
			<div class="modal-footer">
				<a href="#" id="send_otp_sche_join" class="btn btn-success btn-confirm" value="Send OTP">Send OTP</a>
				<a href="#" id="verify_otp_sche_join" disabled class="btn btn-danger btn-confirm" value="Verify OTP">Verify OTP</a>
				<button type="button" class="btn btn-warning" data-dismiss="modal">Close</button>
			</div>
		</div>
	</div>
...
<!-- / modal -->
<!--modal for scheme join otp   ---- otp-- Ends here-->