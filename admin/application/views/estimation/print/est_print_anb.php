...
.top_info {
	...
	font-size: 0; /* set the font-size to 0 */
...
.top_info .rows {
	...
	font-size: 12px; /* set the font-size to the desired value */
		...
		<div class="printable"> 
			...
			<div class="top_head">
				...
				<div class="top_info">
					...
					<div class="rates">
						...
						<div class="rows">
							<div class="left_col">Gold Rate : </div> <div class="right_col"> <?php echo number_format($estimation['goldrate_22ct'],2,'.','') ?></div>
						</div>
						
						<!-- <div class="rows">
							<div class="left_col">Gold Rate : </div> <div class="right_col"> <?php echo number_format($metal_rates['goldrate_18ct'],2,'.','') ?></div>
						</div> -->
			...
			<?php if(sizeof($est_other_item['item_details'])){?>
				...
				<tr>
					...
					<tr>
        					...
        					<td  class="alignCenter" style="width:20%">GWT</td>

							<td  class="alignRight" style="width:20%">NWT</td>

							<td  class="alignRight" style="width:20%">VA</td>
							
							<td  class="alignRight" style="width:20%">MC</td>

        					<td  class="alignRight" style="width:30%">AMOUNT</td>
				...
				<?php 
					...
					<tr>
						...
						<?php //echo ($items['purname'] != "" ? "(". $items['purname'].")" : ""); ?>
							...
							if($items['calculation_based_on'] != 3 && $items['calculation_based_on'] != 4) { ?> @ <?php echo $items['est_rate_per_grm']; } 
							if($items['tag_code'] != '') { ?><br><span style="margin-left:15px;"><?php echo $items['tag_code']; ?></span><?php }
							?>
					...
					<tr>
						...
						<!-- <td class="alignRight">
							...
							// if($items['net_wt'] >= 1) {
								...
								// }
								$va_g = round(($wastage_value + $making_charge),0);
								echo $va_g > 0 ? $va_g : "";
							?> 
						</td> -->
						...
						<?php							
								...
								// $va_g = round(($wastage_value + $making_charge) / $items['est_rate_per_grm'], 3);
								$va_g = ($wastage_value / $items['est_rate_per_grm']);
								// $va_percentage = round(($va_g / $items['net_wt']) * 100, 2);
								$va_percentage = round(($items['wastage_percent']), 2);
								
								// $va_output = $items['va_show'] == 1
								// 		? ($va_percentage > 0 ? $va_percentage . '%' : '') 
								// 		: ($va_g > 0 ? $va_g . "g" : "");
								$va_output = ($va_g > 0 ? $va_g: "");
						?>
						<td class="alignRight"><?php echo number_format($wast_wgt, 3, '.', ''). 'g'; ?></td>
						<!-- <td class="alignRight"><?php echo $making_charge; ?></td> -->
						<td class="alignRight"><?php echo number_format($making_charge, 2, '.', ''); ?></td>
						
						<td class="alignRight"><?php echo moneyFormatIndia(number_format($items['item_cost']-$charge_price-$items['item_total_tax']-$tag_other_itm_amount-$stone_price,2,'.',''));?></td>
					...
					<?php 
						...
						<tr>
							...
							<td colspan="5"><?php echo $other_metal_details['metal_name'].' - (Rs. '.(moneyFormatIndia($other_metal_details['tag_other_itm_rate'])).'/GM)' ?></td>
					...
					<?php foreach($items['stone_details'] as $stone) { 
						...
						?>
								...
								<td colspan="2"><?php echo ' @ ' . $stone['rate_per_gram'] . '/- ' .number_format($stone['wt'], 3, '.', '') . $stone['uom_short_code'] ?></td>
					...
					<?php } else { ?>
						...
						<td colspan="2">IGST <?php echo $items['tax_percentage']?>%</td>

						<td></td>

						<td class="alignRight" colspan="2"><?php echo moneyFormatIndia(number_format($total_tax,2,'.',''));?></td>
					...
					<?php if($chit_amount>0){?>
...

						{
							...
							<?php 
...

								{
                                    ...
                                    if($chit['closing_weight']>0){
    									...
    									$weight_amount = ($chit['rate_per_gram'] != 0 ?  round($chit['closing_weight'] * $chit['rate_per_gram'],0) : round($chit['closing_weight'] * $estimation['goldrate_22ct'],0));
										// $total_weight_amount += number_format(($chit['closing_weight'] * $estimation['goldrate_22ct'])+$chit['additional_benefits'], 2, '.', '');
										$total_weight_amount += number_format($weight_amount + $chit['additional_benefits'], 2, '.', '');    									
    									?>
    								  <tr>
										...
										<td class="alignRight"><?php echo moneyFormatIndia(number_format(round((float)$weight_amount,0),2,'.',''));?></td>
							  ...
							  <?php   }else{
										...
										<td class="alignRight"><?php echo moneyFormatIndia(number_format($weight_amount - $chit['additional_benefits'],2,'.',''));?></td>
								...
								?>
									...
									<tr>
										...
										<td class="alignRight"><?php echo moneyFormatIndia(number_format($wastage_amount,2,'.',''));?></td>
							...
							<?php $i++; } else if($chit['scheme_type']==0){ ?>
								...
								<tr>
									...
									<td class="alignRight"><?php echo moneyFormatIndia(number_format($chit['utl_amount'] - $chit['additional_benefits'],2,'.',''));?></td>
						...
						<?php 
							...
							$total_chit_amount = number_format($total_weight_amount+$total_making_charge_amount+$total_wastage_amount+$total_chit_amount_scheme-$closing_add_chgs,2,'.','')
			...
			<table style="width: 100%">
				...
				<tr class="heading">
					...
					<th width="20%" class="alignLeft">PURITY</th>
				...
				<tbody>
					...
					<tr class="item_name heading"  style="font-weight: normal;">
					...
					<tr  style="font-weight: bold;">
						...
						<td class="alignRight"><?php echo $data['purid'];?></td>
					...
					<tr>
						...
						<td colspan="3"><?php echo ($data['dust_wt'] + $data['wastage_wt'])." / ".$data['stone_wt']; ?></td>
				...
				<tr class="rowClass heading">
					...
					<td class="alignRight" colspan="2"><?php echo number_format($net_wt,3,'.','');?></td>
			...
			<table style="width: 100%">
				...
				<tbody>
					...
					<tr>
						...
						<td class="alignRight"> <?php echo moneyFormatIndia(number_format(round((float)$total_chit_amount,0),2,'.',''));?></td>
			...
			<div>
				<table style="width: 100%; margin-top:4cm;">
					<thead>
						...
						<tr class="heading">
							<th colspan="2" style="text-transform:uppercase; padding: 5px 0;" class="alignLeft">Payment / Order Advance Details</th>
						</tr>
						<tr>
					...
					<tbody>
						<tr>
							<td width="25%" style="padding-top: 20px; vertical-align: bottom;">CASH :</td>
							<td style="border-bottom:1px dashed black; vertical-align: bottom;"></td>
						</tr>
						<tr>
							<td width="25%" style="padding-top: 20px; vertical-align: bottom;">CARD :</td>
							<td style="border-bottom:1px dashed black; vertical-align: bottom;"></td>
						</tr>
						<tr>
							<td width="25%" style="padding-top: 20px; vertical-align: bottom;">UPI :</td>
							<td style="border-bottom:1px dashed black; vertical-align: bottom;"></td>
						</tr>
					</tbody>
				</table>
