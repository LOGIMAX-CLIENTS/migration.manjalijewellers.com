...
//         print_r($this->db->last_query());exit;
    ...
    public function dayDurationSchemeService($id_scheme = "", $id_scheme_account = "")
    {
        ...
        $payments = $this->db->query("SELECT p.id_payment,p.id_scheme_account,date(p.date_payment) as date_payment,sa.start_date,p.due_type,p.due_date,p.grace_date,p.is_limit_exceed ,IFNULL(s.max_chance,0) as max_chance,s.allow_advance_in,s.allow_unpaid_in,s.allow_advance,s.allow_unpaid,if(s.allow_unpaid=1,s.unpaid_months,0) as allow_unpaid_months,if(s.allow_advance=1,s.advance_months,0) as advance_months,
		s.installment_cycle,s.pay_duration,s.payment_chances,s.scheme_type
                                FROM payment p 
                                ...
                                " . ($id_scheme_account > 0 ? "and sa.id_scheme_account = " . $id_scheme_account : '') . "
                                 ...
                                 order by p.id_scheme_account,p.date_payment asc
        ...
        foreach ($payments as $pay) {
            ...
            if (sizeof($dueData[0]) > 0) {
                ...
                // echo "DAYS DURATION UPDATED FOR >>> " . json_encode($dueData[0]);
				echo "DAYS DURATION UPDATED FOR >>> " . json_encode($cycle_data);
            }
        }
    }
   ...
   //RAHUL 30/07
   	function checkTransExist($transactionId,$transStatus){
		$result=[];
		 $sql = "SELECT id_payment,payu_id,ref_trans_id FROM payment WHERE ref_trans_id = ? AND payment_status = 1";
    	$query = $this->db->query($sql, array($transactionId));
		if ($query->num_rows() > 0 ) {
			$result = $query->result_array(); // multiple rows as array of arrays
			$result['status'] = false;
			return $result;
		} else {
			$result['status'] = true;
...

	{
		...
		/* WebHook --2025-07-18 18:18:38 -- : {"data":{"order":{"order_id":"1752834002687a1fd29d90d","order_amount":1000,"order_currency":"INR","order_tags":null},"payment":{"cf_payment_id":5114919317134,"payment_status":"SUCCESS","payment_amount":1000,"payment_currency":"INR","payment_message":"Simulated response message","payment_time":"2025-07-18T15:50:17+05:30","bank_reference":"5114919317134","auth_id":null,"payment_method":{"app":{"channel":null,"upi_id":null}},"payment_group":"wallet"},"customer_details":{"customer_name":"ESAKKIRAJA N","customer_id":"35556","customer_email":"X8BWI3OY@gmail.com","customer_phone":"7010180663"},"charges_details":{"service_charge":19,"service_tax":3.4199999999999999289457264239899814128875732421875,"settlement_amount":977.5800000000000409272615797817707061767578125,"settlement_currency":"INR","service_charge_discount":null}},"event_time":"2025-07-18T15:50:45+05:30","type":"PAYMENT_CHARGES_WEBHOOK"} */
		...
		$log_path = $this->log_dir . '/cashfree/web_hook' . date("Y-m-d") . '.txt';
		...
		/* $cf_response = $data[0];// By default set first array as cf_response[which is last transaction data for the ref_trans_id]

        if(sizeof($data) > 0){ // Check size of response and update cf_response data if payment is success
		...
		$transactionDetail = $this->checkTransExist($transactionId,$transStatus);

		$transactionValid = $transactionDetail['status'];
        if(!empty($transactionId) && $transactionValid){
    		$gatewayResponse = array(
				"payment_mode"       => (($paymentMode) == "credit_card" ? "CC" : (($paymentMode) == "debit_card" ? "DC" : (($paymentMode) == "net_banking" ? "NB" : (($paymentMode) == "na" ? "NA" : $this->update_paymentMode($paymentMode))))),

    			"payu_id"            => $orderId,
    			"remark"             => "WebHook update on " . date('Y-m-d H:i:s'),
    			"payment_ref_number" => $cfPayId,
    			"payment_status"     => ($transStatus == 'success' || $transStatus == 'captured' ? 1 : ($transStatus == 'usercancelled' || $transStatus == 'user_dropped' || $transStatus == 'dropped' || $transStatus == 'bounced' ? 4 : ($transStatus == 'failed' ? 3 : ($transStatus == 'refund' ? 6 : 7))))
    		);
    
    
    		$payment = $this->chit_transaction_model->updateGatewayResponse($gatewayResponse, $transactionId);
        } 
		else if(!$transactionValid){
			$resultArr['status'][] = array("success" => TRUE,  "message" => "Payment ref number Already succeed : " . $transactionId, 'data' => $data, 'transactionDetail'=>$transactionDetail);
			if (!is_dir($this->log_dir . '/cashfree')) {
				mkdir($this->log_dir . '/cashfree', 0777, true);
			}
			$log_path = $this->log_dir . '/cashfree/web_hook_already_succeed' . date("Y-m-d") . '.txt';
			$lg_data = "\n WebHook --" . date('Y-m-d H:i:s') . " -- : " . json_encode($resultArr, true);
			file_put_contents($log_path, $lg_data, FILE_APPEND | LOCK_EX);
			...
			$gatewayResponse = array(
    			...
    			"payu_id"            => $orderId,
    			"remark"             => "WebHook already succeed - update on " . date('Y-m-d H:i:s'),
    			"payment_ref_number" => $cfPayId,
    			"payment_status"     => ($transStatus == 'success' || $transStatus == 'captured' ? 1 :($transStatus == 'usercancelled' || $transStatus == 'user_dropped' || $transStatus == 'dropped' || $transStatus == 'bounced' ? 4 : ($transStatus == 'failed' ? 3 : ($transStatus == 'refund' ? 6 : 7)))),
				"is_gateway_verified" => 1
    		);
			$payment = $this->chit_transaction_model->updateNotValidTransactions($gatewayResponse,$transactionDetail);

			$log_path = $this->log_dir . '/cashfree/trnsaction_id_already_succeed_updated' . date("Y-m-d") . '.txt';
			$lg_data = "\n trnsaction_id__already_succeed_updated --" . date('Y-m-d H:i:s') . " -- : " . json_encode($payment, true);
			file_put_contents($log_path, $lg_data, FILE_APPEND | LOCK_EX);		
		}
	...
	/* CashFree verify cancel payments Esakki- 2025-07-24 */
	function cf_verifyCancel_payments()
	{
		$pg_code = 4;
		$id_branch = '';
		$model =	self::PAY_MODEL;
		...
		$getdata	= array(
			...
			"pg_code"	=> $pg_code,
			"id_branch"	=> '',
			"id_pg"		=> $gateway_info['id_pg'],
			"fromDate"	=> date('Y-m-d', strtotime("-3 days")),
			"toDate"	=> date('Y-m-d', strtotime("0 days"))
		);
		$data['transData']  = $this->chit_transaction_model->getPendCancelPayments($getdata['fromDate'], $getdata['toDate'], $getdata['id_branch'], $getdata['id_pg']);
		// print_r($this->db->last_query());exit;
		if (!is_dir($this->log_dir . date("Y-m-d") . '/cashfree')) {
			mkdir($this->log_dir . date("Y-m-d") . '/cashfree', 0777, true);
		}
		$log_path 	= $this->log_dir . date("Y-m-d") . '/cashfree/cf_cancel_payments_postData-' . date("Y-m-d") . '.txt';
		$ldata 		= "\n" . date('d-m-Y H:i:s') . " \n PostData : " . json_encode($data['transData'], true);
		file_put_contents($log_path, $ldata, FILE_APPEND | LOCK_EX);
		$vCount = 0;
		$resultArr["success_count"] = $vCount;
		if (sizeof($data['transData']) > 0) {
			foreach ($data['transData'] as $tran) {
				$secretKey      = $gateway_info['param_1'];
				$appId          = $gateway_info['param_3'];
				// $api_url 		= $gateway_info['api_url'];
				 $api_url = $gateway_info['api_url'];
				$trans_id	= $tran['txn_id'];
				$postData = "appId=" . $appId . "&secretKey=" . $secretKey . "&orderId=" . $trans_id;
				/* $url_test =$api_url . "/" . $trans_id . "/payments";
				print_r($url_test);exit; */
				$curl = curl_init();
				curl_setopt_array($curl, array(
					CURLOPT_URL => $api_url . "/" . $trans_id . "/payments",
					CURLOPT_RETURNTRANSFER => true,
					CURLOPT_ENCODING => '',
					CURLOPT_MAXREDIRS => 10,
					CURLOPT_TIMEOUT => 8,
				...
				if ($err) {
					...
					$response = json_decode($pay_response);
					$log_path 	= $this->log_dir . date("Y-m-d") . '/cashfree/cf_cancel_payments_response-' . date("Y-m-d") . '.txt';
					$ldata 		= "\n" . date('d-m-Y H:i:s') . " \n Gateway Response : " . json_encode($response, true);
					file_put_contents($log_path, $ldata, FILE_APPEND | LOCK_EX);
					if (empty($response) || $response->code == 'order_not_found') {
						$pay_status	= 'CANCELLED';
						$gwData 	= array(
							"remark"             => "Cancel VerifyService(Cash Free) - " . date('Y-m-d_H-i-s'),
							"is_gateway_verified" => 1,
							"payment_status"     => 4,
							"dev_remark" 		=> (!empty($response) ? $response->code : 'No response from gateway')
						);
						$result = $this->chit_transaction_model->updateGatewayResponse($gwData, $trans_id);
						if ($result['status']) {
							$vCount = $vCount + 1;
							$resultArr['status'][] = array("success" => TRUE,  "message" => "Updated payment status : " . $pay_status, 'data' => $trans_id);
						} else {
							$resultArr['status'][] = array("success" => FALSE, "message" => $result['message'], 'data' => $trans_id);
						}
					}
	...
	public function dayDurationSchemeService($id_scheme = "", $id_scheme_account = "")
	{
		...
		foreach ($payments as $pay) {
			...
			if (sizeof($dueData[0]) > 0) {
				...
				if($pay['is_topup_scheme'] == 1 && $pay['installment_cycle'] == 4){
					$cycle_data['due_type'] = (!empty($dueData[0]['due_type']) ? $dueData[0]['due_type'] : 'ND');
					$cycle_data['topup_slab_data'] = json_encode(
														array('slab_day_start' => (isset($dueData[0]['range_from']) ? $dueData[0]['range_from'] : 0),
																	 'slab_day_end' => (isset($dueData[0]['range_to']) ? $dueData[0]['range_to'] : 0),
																	 'slab_value' => (isset($dueData[0]['payable_value']) ? $dueData[0]['payable_value'] : 0),
																	 'slab_type' => (isset($dueData[0]['payable_type']) ? $dueData[0]['payable_type'] : 0),
																)
													);
				}
