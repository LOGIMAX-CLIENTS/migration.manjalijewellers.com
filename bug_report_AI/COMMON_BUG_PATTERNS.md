...
### PAT-UI-003 — Print Template Dual Display / Spacing Collapsing Gaps

**Category**: UI/Display | **Severity**: P1

**Description**: In dynamic print templates, multiple related layout sections (e.g. Chit Adjustments vs Pre-Close Details) render simultaneously or leave empty table borders/labels and large blank spaces on the canvas. This is caused when mapping helpers populate both sections from a shared data source without filtering by context, or when the templates bind multiple distinct sections to the same global visibility condition.

**Detection Rule**:
```command
# Look for print helper mappings populating distinct keys from the same collection
grep -rn "chit_pre_close_items.*=" admin/application/helpers/template_receipt_helper.php
```

**Fix Template**:
```php
// BEFORE (populates both unconditionally):
if (!empty($items_data['chit_details'])) {
    // populates chit_pre_close_items
}
if (!empty($items_data['chit_details'])) {
    // populates chit_general_items
}

// AFTER (context-aware population):
...
**Modules Found In**: Billing (BIL-CLT07)
