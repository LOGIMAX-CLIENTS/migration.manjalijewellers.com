...
**Formula**: Only 6 `form_validation` / `set_rules` patterns in 23,579 lines — most entities accept ANY POST data without validation
**Implementation**: The few validations appear to be for karigar OTP and financial year entries
**Risk**: ⚠️ Most entity CRUD operations have **zero** server-side validation — data quality depends entirely on client-side form validation which can be bypassed
