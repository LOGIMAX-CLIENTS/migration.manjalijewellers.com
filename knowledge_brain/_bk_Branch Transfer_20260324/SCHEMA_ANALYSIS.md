...
> **Updated**: Round 9 — 2026-03-24
> Added 2 previously missing referenced tables discovered in R8 FLOW_RISK_MATRIX analysis. Part B count: 26 → 28.
> **Original**: Round 2 — 2026-03-11. Source: `ret_brntransfer_model.php` table references + controller INSERT/UPDATE patterns
...
| 27 | `ret_billing_item_stones` | `get_purchase_items()` L1553, `get_partly_sale_details()` L1642, `get_sales_ret_details()` L1704, `get_purchase_items_details()` L1804/1832/1866 | Stone/diamond weight per bill line — used to calculate `dia_wt` in OM/SR/PS transfer items |
| 28 | `ret_partlysold` | Dead-code stub `get_partly_sale_details(4-param)` L1317 — **commented-out block only** | Partly-sold weight tracking — captures `sold_gross_wt` / `sold_net_wt` from prior partial sales. ⚠️ Not used in active code (5-param version replaced with direct subquery) |
