...
## 1. Module Overview
* **Controller**: `admin_ret_brntransfer.php` (1,386 lines, 12 methods + 18 switch sub-routes) — Handles branch transfer CRUD and approval logic.
* **Model**: `ret_brntransfer_model.php` (2,248 lines, 56 methods incl. constructor) — Primary database interactions. ⚠️ ~300 lines are dead-code comment-blocked deprecated method stubs (L1155–L1458).
* **JS**: `ret_branch_transfer.js` (8,786 lines, 51 named functions, 30 internal + 7 cross-module AJAX calls) — Client-side validation, dynamic table management, and AJAX submissions.
...
* **DB Tables**: 5 owned + 26 referenced = 31 total tables touched (see SCHEMA_ANALYSIS.md).
