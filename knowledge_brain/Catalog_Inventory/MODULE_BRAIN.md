...
> **🔄 UPGRADED BRAIN**
> Previous version: Round 7 (2026-03-24)
> Upgraded to: Round 8
> Upgrade date: 2026-03-24
> Changes: 0 methods added, 0 removed, 0 tables changed. View count corrected (89→83 catalog-owned). Upgrade header added. All metrics re-verified against live code.

> **Built**: 2026-03-13 | **Round**: 8 (Upgraded 2026-03-24) | **Coverage**: See COVERAGE_TRACKER.md
...
| Views (master — owned) | `admin/application/views/master/{entity}/` | 79 files | All catalog-entity master views |

> ⚠️ **R8 Correction**: `master/` contains 150 total view files across all modules. Only 79 files in 51 subdirs are owned by Catalog_Inventory controller. R7 incorrectly documented 89. Non-catalog dirs in master/ include: crm_scheme, customer, employee, wallet, scheme, gift, offer, discount, city, country, etc. — these belong to other controllers sharing the view directory.
...
9. **Financial year status toggle NOT in transaction** — `financial_status()` at L9628 lacks `trans_begin/commit` wrapping (RULE-CAT-021)
10. **`fixed_rate` duplicate key** — Design save at L7867-7869 assigns same value twice (RULE-CAT-022)

### 🟢 Low
11. **Image paths hardcoded** — Category images go to `assets/img/ret_category/`, product images handled via `set_image_ret()`
12. **Web catalog vs Retail catalog** — Two parallel product systems (`product` table for web, `ret_product_master` for retail) independently managed

## 9. Codebase Notes

- **Largest controller in the codebase** at 23,584 lines — candidate for refactoring into sub-controllers
...
- `master/` directory shared with all other modules — Catalog_Inventory owns only 83 of 154 files (54%)
