...
> **Round 10 (Upgrade)** — 2026-03-24
> JS massively cleaned: 8,786 → 4,483 lines (all old commented-out duplicate function blocks removed). Controller 1,386 → 1,375 lines. Model 2,248 → 2,137 lines active (dead-code stub block L1155–L1458 still present). All method counts (12 / 56 / 51 / 38) unchanged.

---

## 1. Controller Methods (`admin_ret_brntransfer.php` — 1,375 lines, 12 methods)
...
| 4 | `verify_otp` | L1151–1167 | `otp_logs` | — | `verify_otp()` |
| 5 | `send_otp` | L1169–1212 | `ret_settings` | `otp_logs` | `send_otp()` |
| 6 | `send_sms` | L1214–1221 | — | — | Internal utility (called by `send_otp`, `send_other_issue_otp`) |
| 7 | `bt_get_branches` | L1223–1226 | `ret_branches` | — | `getBTBranches()` |
| 8 | `update_branch_transfer_cancel` | L1262–1293 | — | `ret_branch_transfer` | `update_branch_transfer_cancel()` |
| 9 | `send_other_issue_otp` | L1295–1333 | `ret_settings` | `otp_logs` | `send_other_issue_otp()` |
| 10 | `verify_other_issue_otp` | L1334–1372 | `otp_logs` | — | `verify_otherissue_otp()` |
...
## 2. Model Methods (`ret_brntransfer_model.php` — 2,137 lines active, 56 methods incl. constructor)

> ⚠️ **DEAD CODE — 5 deprecated method stubs** at L1155–L1458 (wrapped in `/*...*/` block): `get_purchase_items` (old 3-param at L1155), `get_partly_sale_details` (old 4-param at L1296), `get_sales_ret_details` (old 4-param at L1336), `old_metal_bill_details` (old 4-param at L1364), `get_purchase_items_details` (old 3-param at L1390). The active 5-param versions (with `$bill_type` EDA filter) replaced them at L1462+. Stubs are harmless but bloat the file by ~300 lines (Anti-Pattern #22).
>
> ✅ **R10 UPGRADE CHANGE**: `get_purchase_items()` (active, L1462–1600) upgraded — switched from Gold/Silver hardcode to `SELECT * FROM metal WHERE metal_status=1` (metal-agnostic, supports future metals). `get_partly_sale_details()` (L1602) now returns `dia_wt` (subquery via `ret_billing_item_stones` + `ret_taging_stone`). All 3 PS/SR/OM methods now receive and filter by `$data['bill_type']`.
...
## 3. JS → Controller AJAX Map (`ret_branch_transfer.js` — 4,483 lines, 51 named functions)

> ✅ **R10 UPGRADE**: JS reduced from 8,786 → 4,483 lines. Removed old commented-out/dead duplicate function blocks: `getTagSearchList` (old version at old L3094), `send_otp` (old at old L4772), `getEstiTags` (old at old L3489/L3613), `remove_brn_row` comment block (old L3421). All 51 active functions preserved. 38 AJAX endpoints unchanged.

### 3a. Internal Endpoints (same controller: `admin_ret_brntransfer`)

| # | JS Function | JS Line (R10) | AJAX URL | Controller Target | HTTP |
|---|---|---|---|---|---|
| 1 | `getTagSearchList()` | L3218 | `.../branch_transfer/getTagsByFilter` | `branch_transfer('getTagsByFilter')` | POST |
| 2 | `getNonTaggedItem()` | L2919 | `.../branch_transfer/getNonTaggedItem` | `branch_transfer('getNonTaggedItem')` | POST |
| 3 | `getEstiTags()` | L3789 | `.../branch_transfer/getEstiTagsByFilter` | `branch_transfer('getEstiTagsByFilter')` | POST |
| 4 | `add_to_trans()` | L3886 | `.../branch_transfer/save` | `branch_transfer('save')` | POST |
| 5 | `get_brantranTagged()` | L4124 | `.../branch_transfer/download_pending` | `branch_transfer('download_pending')` | POST |
| 6 | `get_brantranTagged()` | L4124 | `.../branch_transfer/approval_pending` | `branch_transfer('approval_pending')` | POST |
| 7 | `brnTransUpdStatus()` | L4734 | `.../branch_transfer/updateStatus` | `branch_transfer('updateStatus')` | POST |
| 8 | `send_otp()` | L4842 | `.../send_otp` | `send_otp()` | POST |
| 9 | `send_otp()` (2nd block) | L4842+ | `.../send_otp` | `send_otp()` | POST |
| 10 | `verify_otp()` | L4974 | `.../verify_otp` | `verify_otp()` | POST |
| 11 | `getBTBranches()` | L5042 | `.../bt_get_branches` | `bt_get_branches()` | GET |
| 12 | `update_branch_transfer_cancel()` | L5368 | `.../update_branch_transfer_cancel` | `update_branch_transfer_cancel()` | POST |
| 13 | `get_ajaxBranchTransferlist()` | L5410 | `.../branch_transfer/ajax` | `branch_transfer('ajax')` | POST |
| 14 | `send_other_issue_otp()` | L5593 | `.../send_other_issue_otp` | `send_other_issue_otp()` | POST |
| 15 | `verify_otherissue_otp()` | L5659 | `.../verify_other_issue_otp` | `verify_other_issue_otp()` | POST |
| 16 | `get_purchase_items()` | L6297 | `.../get_purchase_items` | `get_purchase_items()` | POST |
| 17 | `get_brantranNonTagged()` | L4518 | `.../branch_transfer/approval_pending` | `branch_transfer('approval_pending')` | POST |
| 18 | `get_brantranOldMetal()` | L6809 | `.../branch_transfer/approval_pending` | `branch_transfer('approval_pending')` | POST |
| 19 | `get_brantranPackagingItems()` | L7182 | `.../branch_transfer/approval_pending` | `branch_transfer('approval_pending')` | POST |
| 20 | `getRepairOrderDetails()` | L7338 | `.../branch_transfer/getRepairOrderDetails` | `branch_transfer('getRepairOrderDetails')` | POST |
| 21 | `getscan_TagSearchList()` | L7804 | `.../branch_transfer/update_TagsByFilter_scan` | `branch_transfer('update_TagsByFilter_scan')` | POST |
| 22 | `getSearchDesign()` | L2846 | `.../branch_transfer/getDesignByFilter` | `branch_transfer('getDesignByFilter')` | POST |
| 23 | `get_received_lots()` | L2872 | `.../branch_transfer/getLotsByBranch` | `branch_transfer('getLotsByBranch')` | POST |
| 24 | `get_branch_transfer_download()` | L2273 | `.../branch_transfer/download_pending` | `branch_transfer('download_pending')` | POST |
| 25 | `get_branchtranOrderDetails()` | L7548 | `.../branch_transfer/approval_pending` | `branch_transfer('approval_pending')` | POST |
| 26 | `getNonTagReceiptedLots()` | L7992 | `.../getNonTagReceiptedLots` | `getNonTagReceiptedLots()` | POST |
| 27 | `getNonTaggedReceiptedItem()` | L8050 | `.../branch_transfer/getNonTaggedReceiptedItem` | `branch_transfer(...)` | POST |
| 28 | `send_mobile_approval_request()` | L8174 | `.../admin_app_api/bt_approval_otp_req` | **Cross-module** | POST |
| 29 | `update_aprvl_status()` | L8564 | `.../admin_app_api/update_aprvl_status` | **Cross-module** | POST |
| 30 | `get_approval_status()` | L8648 | `.../admin_app_api/get_approval_status` | **Cross-module** | POST |
...
| Function | Lines (R10) | Purpose |
|---|---|---|
| `calcTaggedApprList()` | L593–640 | Calculates totals for tagged approval selection |
...
| `remove_brn_row()` | L3429–3488 | Removes tag row from grid + recalculates |
| `add_to_trans()` | L3886–4011 | Collects form data → submits save AJAX |
| `set_brantranTagged()` | L4256–4517 | Renders tagged items in approval DataTable |
| `calculate_old_metal()` | L6253–6296 | Sums old metal/SR/PS checked items |
| `set_purchase_items_preview()` | L6475–6696 | Renders old metal preview DataTable |
| `fnFormatRowDetails()` | L4012–4123 | Expands row in approval list (tagged detail) |
| `fnFormatRowBillDetails()` | L6697–6808 | Expands row for bill details (old metal) |
| `create_new_empty_order_detail_row()` | L7394–7469 | Adds new order row to table |
| `calculateOrdertotal()` | L7486–7547 | Order total calculation |
| `branch_download_by_scan()` | L7722–7803 | Scan-based download |
| `check_duplicates()` | L8432–8507 | Check duplicate tag in transfer |
| `show_countdown()` | L8728–8765 | OTP countdown timer |
| `set_expiry_status()` | L8766–8880 | OTP expiry handling |
| `remove_row(curRow)` | L7470–7485 | Removes order detail row from grid + recalculates via `calculateOrdertotal()` |
| `set_brantranOrders(data)` | L7598–7721 | Renders order items in approval DataTable (`#bt_approval_list_orders`). Day Close validation inline. |

> **Total named functions: 51** — 30 internal AJAX + 7 cross-module AJAX + 4 overlap (counted in both 3a+3b) + 14 utility = 51 unique. ✅ R10 line numbers updated to match 4,483-line JS.
...
| `ret_billing_item_stones` | `get_purchase_items` (L1553 active), `get_partly_sale_details` (L1642+), `get_sales_ret_details` (L1704+), `get_purchase_items_details` (L1804+) | — |
| `metal` | `get_purchase_items` (L1466 — `SELECT * FROM metal WHERE metal_status=1`) | — |
