...
> **Last Updated**: 2026-03-24 — Round 11 (Upgrade Pass 2 — Remaining Files)
...
|---|---|---|---|---|
| Controller methods | 12 | 12 | 100% | 🔵 Verified R10 |
| Controller sub-routes | 18 | 18 | 100% | 🔵 Verified |
| Model methods | 56 | 56 | 100% | 🔵 Verified R10 |
| JS named functions | 51 | 51 | 100% | 🔵 Verified R10 ← JS now 4,483 lines (was 8,786 − 4,303 lines of dead duplicate blocks cleaned) |
| JS AJAX `url:` patterns | 38 | 38 | 100% | 🔵 Verified R10 |
| DB tables (owned) | 5 | 5 | 100% | 🔵 Verified |
| DB tables (referenced) | 29 | 29 | 100% | 🔵 Updated R10 ← was 28; added `metal` table (now used in `get_purchase_items`) |
| Table→Method reverse map | 30 | 30 | 100% | 🔵 Verified |
| Business rules | 16 | — | — | 🟢 Updated R11 ← was 15; added RULE-BRT-016 (metal-agnostic PS/SR/OM loop) |
...
### Round 10 — 2026-03-24 (Upgrade — Full Code Diff vs R9)
**Focus**: Detect all code changes since R9. Full rescan: controller, model, JS, views.

| Finding | R9 | R10 Actual | Delta | Impact |
|---|---|---|---|---|
| Controller lines | 1,386 | **1,375** | −11 | Minor cleanup/reorganization, no method changes |
| Controller methods | 12 | **12** | 0 | ✅ Unchanged |
| Model lines | 2,248 | **2,137** | −111 | Dead duplicate inline comment blocks trimmed |
| Model methods | 56 | **56** | 0 | ✅ Unchanged |
| JS lines | 8,786 | **4,483** | **−4,303** | Large dead/commented function versions removed: `getTagSearchList` (old), `send_otp` (old), `getEstiTags` (old), `remove_brn_row` comment blocks |
| JS named functions (active) | 51 | **51** | 0 | ✅ Unchanged |
| JS AJAX `url:` | 38 | **38** | 0 | ✅ Unchanged |
| DB referenced tables | 28 | **29** | +1 | Added `metal` table — now used in `get_purchase_items` L1466 |
| `ret_billing_item_stones` status | "active at L1553+" | **CONFIRMED ACTIVE in 4 methods** | corrected | Used in `get_purchase_items`, `get_partly_sale_details`, `get_sales_ret_details`, `get_purchase_items_details` |

**Upgrade findings:**
> ✅ **Model `get_purchase_items()` upgraded**: Replaced Gold/Silver hardcoded if-else with `SELECT * FROM metal WHERE metal_status=1` — now metal-agnostic, supports any future metals.
> ✅ **`get_partly_sale_details()` upgraded**: Added `dia_wt` column (from `ret_billing_item_stones` + `ret_taging_stone` subquery) and `transfered_wt` column. Better data richness for PS transfers.
> ✅ **JS cleaned**: 4,303 lines of dead commented function blocks removed. All 51 active functions, 38 AJAX endpoints preserved. Line numbers updated in METHOD_INDEX.
> ⚠️ **Dead-code stubs L1155–L1458** still present in model (Anti-Pattern #22). No change.
...
| 10 | Upgrade scan (2026-03-24) | Controller 1,386→1,375 lines (−11). Model 2,248→2,137 active lines (−111). JS 8,786→4,483 lines (−4,303 — dead duplicate blocks removed). `metal` table added as #29 referenced. `ret_billing_item_stones` confirmed ACTIVE in 4 methods. `get_purchase_items()` upgraded to metal-agnostic. All method/AJAX counts unchanged (12/56/38/51). |
| 11 | Upgrade Pass 2 (2026-03-24) | DATA_FLOW.md: 6 stale JS line refs corrected. BUSINESS_RULES.md: BRT-005 JS lines updated + RULE-BRT-016 added (metal-agnostic `get_purchase_items`). CROSS_MODULE_MAP.md: `metal` table added as Master Data dependency. Business rules: 15→16. |
