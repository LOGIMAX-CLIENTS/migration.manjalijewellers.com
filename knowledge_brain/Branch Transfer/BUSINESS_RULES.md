...
> JS line refs updated throughout. Added RULE-BRT-016 for metal-agnostic `get_purchase_items` upgrade.
> **R9**: Added BRT-014, BRT-015 from FLOW_RISK_MATRIX contract gap analysis.
...
**JS Functions:** `send_otp()` L4842, `verify_otp()` L4974, `send_other_issue_otp()` L5593, `verify_otherissue_otp()` L5659, `send_mobile_approval_request()` L8174
...
## RULE-BRT-016: Old Metal/PS/SR Transfer Uses Metal-Agnostic Loop (R10 Upgrade)
**Rule:** `get_purchase_items()` (model L1462–1600) now queries `SELECT * FROM metal WHERE metal_status=1` to iterate over all active metals dynamically. All three item categories (Partly Sale, Sales Return, Old Metal) are built per metal row. Prior to R10, the logic was hardcoded to Gold (id_metal=1) and Silver (id_metal=2) only.
**Implementation:** Model `get_purchase_items()` L1466 — `$partlySale = $this->db->query("SELECT * FROM metal WHERE metal_status = 1")`
**Validation:** Server-side only. Metal type as string: `strtolower($items['metal'])` builds `partly_sale_{metal}`, `sales_ret_items_{metal}`, `old_metal_{metal}` type keys.
**Impact:** If a new metal is added to the `metal` table with `metal_status=1`, it will automatically appear in branch transfer OM/PS/SR flow without code changes.
**Discovered:** R10 Upgrade scan — 2026-03-24
