...
> **🔄 UPGRADED BRAIN**
> Previous version: R1 (2026-03-06) — 4,709L / 120 methods ctrl + 2,866L / 178 methods model
> Upgraded to: R2-Upgrade (2026-03-25)
> Changes: +128 controller lines, +3 ctrl methods (KYC), +78 model lines, +4 model methods (KYC), 5 new tables, 3 new bugs
>
> **Module**: System Configuration & Master Data Management  
> **Controller**: `admin_settings.php` (**4,837 lines — 123 methods**)  
> **Model**: `admin_settings_model.php` (**2,944 lines — 182 methods**)  
...
17. **KYC Settings** — `kyc_master`, `kyc_settings`, `kyc_rules`: **NEW** — KYC document requirements per scheme or customer
...
|---|---|---|---|
| `admin/application/controllers/admin_settings.php` | **4,837** | **123** | Mega-controller: all settings & masters |
| `admin/application/models/admin_settings_model.php` | **2,944** | **182** | All DB operations for settings |
...
| `kyc_master` | KYC document types | ✅ This module |
| `kyc_settings` | KYC configuration (required, mode, verify type) | ✅ This module |
| `kyc_rules` | KYC rules per scheme or customer | ✅ This module |
| `ledger_master` | Bank ledger accounts | ✅ This module |
| `ledger_mapping` | Ledger → bank/paymode mappings | ✅ This module |
...
| 2 | **`clear_database()` uses raw `$_POST`** | 🔴 HIGH | Controller L2190 |
| 3 | **`clear_database()` TRUNCATES production tables** | 🔴 CRITICAL | Controller L2190-2252 |
...
| 6 | **No CSRF protection** on `clear_database()` | 🔴 CRITICAL | Controller L2190 |
| 7 | **Email/password in plain text** — mail_settings stores plain password | 🟡 MED | Controller L2412 |
| 8 | **Hardcoded URLs** in commented code — old production URLs visible | 🟡 LOW | Controller L670-673 |
| 9 | **`$general[tab_name]` — undefined constant** used as array key | 🟡 MED | Controller L1957, L2108, L4519 |
| 10 | **Menu generation builds HTML in model** | 🟡 LOW | Model L41-80 |
| 11 | **DB backup creates ZIP with no auth** | 🟡 MED | Controller L2275-2313 |
| 12 | **`rrmdir()` recursively deletes directories** | 🔴 HIGH | Controller L2262-2274 |
| 13 | **No transaction in general_settings Save** | 🟡 MED | Controller L1834-1969 |
| 14 | **version_details() hardcoded CURL URL** — POST to `pm.logimaxindia.com` | 🟡 MED | Controller L4249 |
| 15 | **update_notification_status() raw $status/$id** — SQL injection via GET | 🔴 HIGH | Model L2743 |
| 16 | **getRef_nos() raw SQL concat** — `g_ref_no=` + raw data in WHERE | 🔴 HIGH | Model L2194-2214 |
