...
| 🔄 Metal weight display inconsistent | `formatMetalWeight()` | Check `metal_wgt_digit` helper loaded; check `session.metal_wgt_roundoff/decimal` | Controller:L30,L1562 + JS |
| 🔄 Payment unexpectedly blocked | `scheme_account.disable_payment` | `disable_payment=1` blocks via Model L2297-2302 guard | Model:L2294-2302 |
| 🔄 Quarterly scheme due count wrong | `scheme.installment_cycle=3` | Uses PERIOD_DIFF (monthly calc) but daily date matching for matured schemes — possible mismatch | Model:L6510,L8093 |
