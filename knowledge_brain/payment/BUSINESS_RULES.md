...
## RULE-PAY-029: Centralized Metal Weight Formatting (🔄 NEW in R3)
- **Trigger**: Any metal weight display/storage — replaces all scattered `session.metal_wgt_roundoff == 0 ? trim_decimal() : number_format()` patterns
- **Function**: `formatMetalWeight($value)` — from `metal_wgt_digit` helper (loaded in constructor L30)
- **Behavior**: Delegates to session-driven rounding config (`metal_wgt_roundoff`, `metal_wgt_decimal`)
- **Implementation (PHP)**: Helper `formatMetalWeight()` — used at Controller L1562, L1578
- **Implementation (JS)**: Global `formatMetalWeight()` function — used at ~15 locations throughout `payment.js`, `verify_payment.js`
- **Commit**: `45c218ff` — "Unify metal weight formatting"
- **Risk**: Low — centralizes logic, but any formatting bug now propagates everywhere

## RULE-PAY-030: Disable Payment Guard (🔄 NEW in R3)
- **Trigger**: Before allowing payment on a scheme_account
- **Rule**: After all existing `allow_pay` logic, an additional guard checks:
  ```
  IF allowed_due > 0 AND allow_pay = 'Y' AND paid_ins < total_ins AND disable_payment != 1:
      allow_pay = 'Y'
  ELSE:
      allow_pay = 'N'
  ```
- **Implementation**: Model `get_paymentContent()` L2297-2302
- **Field**: `scheme_account.disable_payment` (1 = blocked)
- **⚠️ Edge cases**: Dev comment says "discuss for APN and multiple payment chances" — incomplete for multi-chance schemes
- **Commit**: `60f1ef8b` — "Disable payment condition updated"
