...
### Dimension 10: Installment Cycle (`scheme.installment_cycle`) — 🔄 UPDATED in R3
| Value | Label | Due Count Calculation | Date Condition for Matured Schemes |
|---|---|---|---|
| 0 | Monthly | `PERIOD_DIFF(CURDATE, start_date) + 1` | `DATE_FORMAT(date_payment,'%Y-%m') = current month` |
| 1 | Daily | `DATEDIFF(CURDATE, start_date) + 1` | `DATE_FORMAT(date_payment,'%Y-%m-%d') = current date` |
| 2 | Weekly | `FLOOR(DATEDIFF(CURDATE, start_date)/7) + 1` | `DATE_FORMAT(date_payment,'%Y-%m-%d') = current date` |
| 3 | **Quarterly** 🔄 NEW | Same as Monthly (`PERIOD_DIFF + 1`) | `DATE_FORMAT(date_payment,'%Y-%m-%d') = current date` |

> **Commit**: `d7c9cb60` — `installment_cycle=3` (quarterly) now treated like monthly for due count, but like daily for matured-scheme date matching. This is potentially a logic mismatch.
...
| 13 | `disable_payment=1` on scheme_account | Payment blocked | 🔄 NEW: Model L2297-2302 overrides `allow_pay` to `'N'`. Dev comment warns about APN + multi-chance conflicts | 🟡 |
