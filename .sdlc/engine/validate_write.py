...
# Patterns that are always allowed (pipeline state, spec files, artifacts)
ALWAYS_ALLOWED = [
    '.sdlc/pipeline.json',
    '.sdlc/active/**',
    '.sdlc/done/**',
...
def get_phase_permissions(config_data, phase, sub_role=None):
    """Get the allowed patterns for a phase from config.json file_permissions."""
    file_perms = config_data.get('file_permissions', {})

    phase_perms = file_perms.get(phase)
    if phase_perms and isinstance(phase_perms, list):
        # format: file_permissions.CODING = ["pattern1", "pattern2"]
        return {
            'allowed_patterns': phase_perms,
            'blocked_reason': f'{phase} phase has restricted file permissions.',
        }
    elif phase_perms and isinstance(phase_perms, dict):
        # Extended format with explicit blocked_reason
        return {
    ...
    return {
        'allowed_patterns': [],
        'blocked_reason': f'Phase {phase} has no defined permissions in config.json. Blocked by default.',
    }
...
def validate_write(filepath, phase=None, sub_role=None, project_root=None, quiet=False):
    ...
    if phase is None:
        state = load_json(os.path.join(sdlc_dir, STATE_FILE))
        if state is None:
            return True, 'No pipeline state found — no restrictions active.'

        # format: active_task is a single object (not a list)
        active_task = state.get('active_task')
        if active_task is None:
            return True, 'No active task — no restrictions.'

        phase = active_task.get('phase', 'IDLE')

    # Load config (v2 uses config.json for file permissions, not roles.json)
    config = load_json(os.path.join(sdlc_dir, CONFIG_FILE))
    if config is None:
        return True, 'No config.json found — no restrictions active.'
    ...
    perms = get_phase_permissions(config, phase, sub_role)
    ...
    suggested_phase = suggest_phase(config, rel_path)
...
def suggest_phase(config_data, rel_path):
    ...
    for phase_name, perms in file_perms.items():
        ...
        if phase_name.startswith('_') or phase_name in ('IDLE',):
            continue
        # Handle list format (v2)
